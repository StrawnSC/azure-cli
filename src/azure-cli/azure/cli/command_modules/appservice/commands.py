...
from ._validators import (
    validate_onedeploy_params,
    validate_staticsite_link_function,
    validate_staticsite_sku,
    validate_vnet_integration,
    validate_asp_create,
    validate_functionapp_asp_create,
)
...
def update_function_ex_handler_factory():
    ...
    def _ex_handler(ex):
        ...
        if http_error_response:
            try:
                detail = (
                    "If using '--plan', a consumption plan may be unable to migrate "
...
def load_command_table(self, _):
    ...
    appservice_plan_sdk = CliCommandType(
        operations_tmpl="azure.mgmt.web.operations#AppServicePlansOperations.{}", client_factory=cf_plans
    )
    webapp_sdk = CliCommandType(
        operations_tmpl="azure.mgmt.web.operations#WebAppsOperations.{}", client_factory=cf_webapps
    )

    appservice_custom = CliCommandType(operations_tmpl="azure.cli.command_modules.appservice.custom#{}")

    webapp_access_restrictions = CliCommandType(
        operations_tmpl="azure.cli.command_modules.appservice.access_restrictions#{}"
    )

    appservice_environment = CliCommandType(
        operations_tmpl="azure.cli.command_modules.appservice.appservice_environment#{}"
    )
    ...
    with self.command_group("webapp", webapp_sdk) as g:
        g.custom_command(
            "create", "create_webapp", exception_handler=ex_handler_factory(), validator=validate_vnet_integration
        )
    ...
    with self.command_group("functionapp") as g:
        g.custom_command(
            "create", "create_functionapp", exception_handler=ex_handler_factory(), validator=validate_vnet_integration
        )
        ...
        g.generic_update_command(
            "update",
            getter_name="get_functionapp",
            setter_name="set_functionapp",
            exception_handler=update_function_ex_handler_factory(),
            custom_func_name="update_functionapp",
            getter_type=appservice_custom,
    ...
    with self.command_group(
        "functionapp config access-restriction", custom_command_type=webapp_access_restrictions
    ) as g:
        g.custom_show_command("show", "show_webapp_access_restrictions")
        g.custom_command("add", "add_webapp_access_restriction")
        g.custom_command("remove", "remove_webapp_access_restriction")
