    ...
    def test_logicapp_e2e(self, resource_group):
        ...
        self.cmd(
            "storage account create --name {} -g {} -l {} --sku Standard_LRS".format(
                storage, resource_group, DEFAULT_LOCATION
            )
        )

        self.cmd(
            "logicapp create -g {} -n {} -p {} -s {}".format(resource_group, logicapp_name, plan, storage),
            checks=[
        ...
        self.cmd(
            "logicapp show -g {} -n {}".format(resource_group, logicapp_name),
            checks=[
                JMESPathCheck("name", logicapp_name),
        ...
        self.cmd(
            "logicapp list -g {}".format(resource_group),
            checks=[JMESPathCheck("length([])", 1), JMESPathCheck("[0].name", logicapp_name)],
        )
        ...
        self.cmd(
            "logicapp show -g {} -n {}".format(resource_group, logicapp_name),
            checks=[JMESPathCheck("state", "Running"), JMESPathCheck("name", logicapp_name)],
        )
        ...
        self.cmd(
            "logicapp show -g {} -n {}".format(resource_group, logicapp_name),
            checks=[JMESPathCheck("state", "Stopped"), JMESPathCheck("name", logicapp_name)],
        )
        ...
        self.cmd("logicapp list -g {}".format(resource_group), checks=[JMESPathCheck("length([])", 0)])
...
class LogicAppPlanTest(ScenarioTest):
    def _create_app_service_plan(self, sku, resource_group, plan_name=None, expect_failure=False):
        if plan_name == None:
            ...
            plan = plan_name
        self.cmd(
            "appservice plan create -g {} -n {} --sku {}".format(resource_group, plan, sku),
    ...
    def _create_logic_app(self, plan, resource_group, storage_account):
        ...
        self.cmd(
            "logicapp create -g {} -n {} --plan {} --storage-account {}".format(
                resource_group, name, plan, storage_account
            )
        )
        return name
    ...
    def _validate_logicapp_create(self, name, resource_group, plan):
        ...
        self.cmd(
            "logicapp show -g {} -n {}".format(resource_group, name),
            checks=[
...
class LogicAppOnWindows(ScenarioTest):
    ...
    def test_logicapp_windows(self, resource_group, storage_account):
        logicapp_name = self.create_random_name("logicappwindowsruntime", 40)

        self.cmd(
            "logicapp create -g {} -n {} -c {} -s {} --os-type Windows".format(
                resource_group, logicapp_name, WINDOWS_ASP_LOCATION_LOGICAPP, storage_account
            )
        ).assert_with_checks(
            [
                JMESPathCheck("state", "Running"),
...
class LogicAppOnLinux(ScenarioTest):
    ...
    def test_logicapp_on_linux(self, resource_group, storage_account):
        ...
        logicapp_name = self.create_random_name(prefix="logicapp-linux", length=24)
        self.cmd(
            "appservice plan create -g {} -n {} --sku WS1 --is-linux".format(resource_group, plan),
            checks=[
        ...
        self.cmd(
            "logicapp create -g {} -n {} --plan {} -s {}".format(resource_group, logicapp_name, plan, storage_account),
            checks=[JMESPathCheck("name", logicapp_name)],
        )
        self.cmd(
            "logicapp list -g {}".format(resource_group),
            checks=[
