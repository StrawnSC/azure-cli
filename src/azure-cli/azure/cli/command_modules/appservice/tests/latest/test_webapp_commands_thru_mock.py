...
class TestWebappMocked(unittest.TestCase):
    ...
    def test_set_domain_name(self, client_factory_mock):
        ...
        binding = HostNameBinding(
            location=webapp.location, domain_id=domain, custom_host_name_dns_record_type="A", host_name_type="Managed"
        )
    ...
    def test_get_external_ip_from_ase(self, client_factory_mock):
        ...
        HostNameSslState, SslState, Site, AddressResponse = cmd_mock.get_models(
    ...
    def test_list_publish_profiles_on_slots(self, site_op_mock):
        site_op_mock.return_value = [
            b'<publishData><publishProfile publishUrl="ftp://123"/><publishProfile publishUrl="ftp://1234"/></publishData>'
        ]
        # action
        result = list_publish_profiles(mock.MagicMock(), "myRG", "myweb", "slot1")
        # assert
        site_op_mock.assert_called_with(
            mock.ANY, "myRG", "myweb", "list_publishing_profile_xml_with_secrets", "slot1", {"format": "WebDeploy"}
        )
    ...
    def test_browse_with_trace(self, webbrowser_mock, log_mock, site_op_mock):
        ...
        site.host_name_ssl_states = [HostNameSslState(name="does not matter", ssl_state=SslState.ip_based_enabled)]
    ...
    def test_match_host_names_from_cert(self):
        ...
        result = _match_host_names_from_cert(
            ["*.mysite.com", "mysite.com"], ["admin.mysite.com", "log.mysite.com", "mysite.com"]
        )
    ...
    def test_restore_snapshot(self, client_factory_mock):
        ...
        SnapshotRecoverySource, SnapshotRestoreRequest = cmd_mock.get_models(
            "SnapshotRecoverySource", "SnapshotRestoreRequest"
        )
        source = SnapshotRecoverySource(
            id="/subscriptions/sub1/resourceGroups/src_rg/providers/Microsoft.Web/sites/src_web/slots/src_slot"
        )
        ...
        overwrite_request = SnapshotRestoreRequest(
            overwrite=True, snapshot_time="2018-12-07T02:01:31.4708832Z", recover_configuration=True
        )
    ...
    def test_valid_linux_create_options(self):
        ...
        self.assertTrue(
            validate_container_app_create_options(None, None, test_multi_container_config, test_multi_container_type)
        )
        self.assertFalse(
            validate_container_app_create_options(
                some_runtime, None, test_multi_container_config, test_multi_container_type
            )
        )
        self.assertFalse(validate_container_app_create_options(some_runtime, None, test_multi_container_config, None))
        self.assertFalse(
            validate_container_app_create_options(some_runtime, test_docker_image, test_multi_container_config, None)
        )
    ...
    def test_create_managed_ssl_cert(self, generic_site_op_mock, client_factory_mock, verify_binding_mock):
        ...
        cert_def = Certificate(location="westeurope", canonical_name=host_name, server_farm_id=farm_id, password="")
        client.certificates.create_or_update.assert_called_once_with(
            name=host_name, resource_group_name=rg_name, certificate_envelope=cert_def
        )
