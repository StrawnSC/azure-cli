...
class FunctionAppAccessRestrictionScenarioTest(ScenarioTest):
    ...
    def test_functionapp_access_restriction_show(self, resource_group, location):
        self.kwargs.update({"app_name": self.create_random_name(prefix="cli-funcapp-nwr", length=24), "loc": location})

        self.cmd(
            "functionapp create -g {rg} -n {app_name} --consumption-plan-location {loc} -s {sa}",
            checks=[JMESPathCheck("state", "Running")],
        )
    ...
    def test_functionapp_access_restriction_set_simple(self, resource_group, location):
        ...
        self.cmd(
            ...
            checks=[JMESPathCheck("scmIpSecurityRestrictionsUseMain", True)],
        )
    ...
    def test_functionapp_access_restriction_set_complex(self, resource_group, location):
        ...
        self.cmd(
            ...
            checks=[JMESPathCheck("scmIpSecurityRestrictionsUseMain", False)],
        )

    @ResourceGroupPreparer(
        random_name_length=17, parameter_name_for_location="location", location=WINDOWS_ASP_LOCATION_WEBAPP
    )
    ...
    def test_functionapp_access_restriction_add_service_endpoint(self, resource_group, location):
        ...
        self.cmd(
            "az network vnet create -g {rg} -n {vnet_name} --address-prefixes 10.0.0.0/16 --subnet-name endpoint-subnet --subnet-prefixes 10.0.0.0/24",
            checks=[JMESPathCheck("subnets[0].serviceEndpoints", None)],
        )
    ...
    def test_functionapp_access_restriction_slot(self, resource_group, location):
        ...
        self.cmd(
            "functionapp deployment slot create -g {rg} -n {app_name} --slot {slot_name}",
            checks=[JMESPathCheck("state", "Running")],
        )

        self.cmd(
            "functionapp config access-restriction show -g {rg} -n {app_name} --slot {slot_name}",
            checks=[
                JMESPathCheck("length(@)", 3),
                JMESPathCheck("length(ipSecurityRestrictions)", 1),
                JMESPathCheck("ipSecurityRestrictions[0].name", "Allow all"),
        ...
        self.cmd(
            "functionapp config access-restriction add -g {rg} -n {app_name} --rule-name developers --action Allow --ip-address 130.220.0.0/27 --priority 200 --slot {slot_name}",
            checks=[
                JMESPathCheck("length(@)", 2),
                JMESPathCheck("[0].name", "developers"),
                JMESPathCheck("[0].action", "Allow"),
