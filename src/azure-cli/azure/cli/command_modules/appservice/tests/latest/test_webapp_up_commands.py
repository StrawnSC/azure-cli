...
class WebAppUpE2ETests(ScenarioTest):
    ...
    def test_webapp_up_no_plan_e2e(self, resource_group):
        ...
        self.cmd(
            ...
            checks=[JMESPathCheck("[0].name", "SCM_DO_BUILD_DURING_DEPLOYMENT"), JMESPathCheck("[0].value", "True")],
        )
    ...
    def test_webapp_up_python_e2e(self, resource_group):
        ...
        # test dryrun operation
        result = self.cmd("webapp up -n {} --sku  S1 --dryrun".format(webapp_name)).get_output_in_json()
        ...
        # test the full E2E operation works
        full_result = self.cmd(
            "webapp up -n {} --sku  S1 -g {} --plan {}".format(webapp_name, resource_group, plan)
        ).get_output_in_json()
        self.assertEqual(result["name"], full_result["name"])
    ...
    def test_webapp_up_dotnetcore_e2e(self, resource_group):
        ...
        self.cmd(
            "webapp config show", checks=[JMESPathCheck("tags.cli", "None"), JMESPathCheck("windowsFxVersion", None)]
        )
    ...
    def test_webapp_up_dotnet6_e2e(self, resource_group):
        ...
        # test dryrun operation
        result = self.cmd("webapp up -n {} --dryrun --os-type Linux".format(webapp_name)).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "free")
        self.assertTrue(result["name"].startswith(webapp_name))
        self.assertTrue(result["src_path"].replace(os.sep + os.sep, os.sep), up_working_dir)
    ...
    def test_webapp_up_statichtml_e2e(self, resource_group):
        ...
        zip_file_name = os.path.join(TEST_DIR, "html-static-hello-world-up.zip")
        ...
        # test dryrun operation
        result = self.cmd("webapp up -n {} --dryrun --html".format(webapp_name)).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "free")
        self.assertTrue(result["name"].startswith(webapp_name))
        self.assertTrue(result["src_path"].replace(os.sep + os.sep, os.sep), up_working_dir)
        ...
        # test the full E2E operation works
        full_result = self.cmd(
            "webapp up -n {} -g {} --plan {} --html".format(webapp_name, resource_group, plan)
        ).get_output_in_json()
        self.assertEqual(result["name"], full_result["name"])
        ...
        self.cmd(
            "webapp config appsettings list",
            checks=[JMESPathCheck("[1].name", "SCM_DO_BUILD_DURING_DEPLOYMENT"), JMESPathCheck("[1].value", "True")],
        )
    ...
    def test_webapp_up_name_exists_in_subscription(self, resource_group):
        ...
        self.cmd('webapp create -g {} -n {} --plan {} -r "python|3.7"'.format(resource_group, webapp_name, plan))
    ...
    def test_webapp_up_choose_os(self, resource_group):
        ...
        # test dryrun operation
        result = self.cmd(
            'webapp up -n {} -g {} --plan {} --os-type "linux" --dryrun'.format(webapp_name, resource_group, plan)
        ).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "premiumv2")
        self.assertTrue(result["name"].startswith(webapp_name))
        self.assertTrue(result["src_path"].replace(os.sep + os.sep, os.sep), up_working_dir)
        ...
        # test the full E2E operation works
        full_result = self.cmd(
            'webapp up -n {} -g {} --plan {} --os-type "linux"'.format(webapp_name, resource_group, plan)
        ).get_output_in_json()
        self.assertEqual(result["name"], full_result["name"])
        ...
        self.cmd("webapp config show", checks=[JMESPathCheck("tags.cli", "None")])
    ...
    def test_webapp_up_choose_runtime(self, resource_group):
        ...
        # test dryrun operation
        result = self.cmd(
            'webapp up -n {} -g {} --plan {} --runtime "PYTHON|3.7" --sku S1 --dryrun'.format(
                webapp_name, resource_group, plan
            )
        ).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "standard")
        self.assertTrue(result["name"].startswith(webapp_name))
        ...
        # test the full E2E operation works
        full_result = self.cmd(
            'webapp up -n {} -g {} --plan {} --runtime "PYTHON|3.7" --sku "S1"'.format(
                webapp_name, resource_group, plan
            )
        ).get_output_in_json()
        self.assertEqual(result["name"], full_result["name"])
        ...
        self.cmd(
            "webapp config show",
            checks=[JMESPathCheck("linuxFxVersion", "PYTHON|3.7"), JMESPathCheck("tags.cli", "None")],
        )
    ...
    def test_webapp_up_choose_os_and_runtime(self, resource_group):
        ...
        # test dryrun operation
        result = self.cmd(
            'webapp up -n {} -g {} --plan {} --os "linux" --runtime "node|12-lts" --sku "S1" --dryrun'.format(
                webapp_name, resource_group, plan
            )
        ).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "standard")
        ...
        # test the full E2E operation works
        full_result = self.cmd(
            'webapp up -n {} -g {} --plan {} --os "linux" --runtime "node|12-lts" --sku "S1"'.format(
                webapp_name, resource_group, plan
            )
        ).get_output_in_json()
        self.assertEqual(result["name"], full_result["name"])
    ...
    def test_webapp_up_runtime_delimiters(self, resource_group):
        ...
        # test dryrun operation
        result = self.cmd(
            'webapp up -n {} -g {} --plan {} --os "windows" --runtime "java|1.8|Java SE|8" --sku "S1" --dryrun'.format(
                webapp_name, resource_group, plan
            )
        ).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "standard")
    ...
    def test_linux_to_windows_fail(self, resource_group):
        ...
        with self.assertRaises(CLIError):
            self.cmd(
                'webapp up -n {} -g {} --plan {} --os "windows" --runtime "node|12LTS" --sku "S1"'.format(
                    webapp_name, resource_group, plan
                )
            )
    ...
    def test_windows_to_linux_fail(self, resource_group):
        ...
        # test dryrun operation
        result = self.cmd(
            'webapp up -n {} -g {} --plan {} --os "windows" --runtime "node|12LTS" --sku "S1" --dryrun'.format(
                webapp_name, resource_group, plan
            )
        ).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "standard")
    ...
    def test_webapp_up_change_runtime_version(self, resource_group):
        ...
        time.sleep(30)
        full_result = self.cmd(
            'webapp up -n {} -g {} --plan {} --os "linux" --runtime "node|16-lts" --sku "S1"'.format(
                webapp_name, resource_group, plan
            )
        ).get_output_in_json()
        self.assertEqual(result["name"], full_result["name"])
        ...
        self.cmd(
            "webapp config show",
            checks=[JMESPathCheck("linuxFxVersion", "NODE|16-lts"), JMESPathCheck("tags.cli", "None")],
        )
        ...
        self.cmd(
            "webapp config show",
            checks=[JMESPathCheck("linuxFxVersion", "NODE|12-lts"), JMESPathCheck("tags.cli", "None")],
        )
    ...
    def test_webapp_up_generate_default_name(self, resource_group):
        ...
        # test dryrun operation
        result = self.cmd("webapp up --dryrun").get_output_in_json()
        self.assertEqual(result["sku"].lower(), "premiumv2")
        self.assertTrue(result["src_path"].replace(os.sep + os.sep, os.sep), up_working_dir)
        self.assertEqual(result["runtime_version"].lower(), "node|12-lts")
        self.assertEqual(result["os"].lower(), "linux")
        ...
        # test the full E2E operation works
        self.cmd("webapp up -g {} --plan {}".format(resource_group, plan)).get_output_in_json()
    ...
    def test_webapp_up_linux_windows_sharing_resource_group(self, resource_group):
        ...
        result = self.cmd("webapp up -n {} --sku  S1 --dryrun".format(linux_webapp_name)).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "standard")
        ...
        full_result = self.cmd(
            "webapp up -n {} --sku  S1 -g {} --plan {} --os-type linux".format(
                linux_webapp_name, resource_group, linux_plan
            )
        ).get_output_in_json()
        self.assertEqual(result["name"], full_result["name"])
        ...
        result = self.cmd(
            "webapp up -n {} --sku  S1 --dryrun -r 'node|14lts' --os-type windows --plan {}".format(
                windows_webapp_name, windows_plan
            )
        ).get_output_in_json()
        self.assertEqual(result["sku"].lower(), "standard")
        ...
        full_result = self.cmd(
            "webapp up -n {} --sku  S1 -g {} --plan {}".format(windows_webapp_name, resource_group, windows_plan)
        ).get_output_in_json()
        self.assertEqual(result["name"], full_result["name"])

        # Verify windows app is created
        self.cmd(
            "webapp show -g {} -n {}".format(resource_group, windows_webapp_name),
            checks=[
