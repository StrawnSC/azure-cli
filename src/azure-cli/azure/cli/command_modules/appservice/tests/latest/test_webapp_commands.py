...
class WebappBasicE2ETest(ScenarioTest):
    ...
    def test_webapp_e2e(self, resource_group):
        ...
        self.cmd(
            "appservice plan list -g {}".format(resource_group),
            checks=[
        ...
        # test idempotency
        self.cmd("appservice plan create -g {} -n {} --per-site-scaling".format(resource_group, plan))
        self.cmd(
            "appservice plan list -g {}".format(resource_group),
            checks=[
                JMESPathCheck("length(@)", 1),
        ...
        self.cmd(
            "appservice plan list -g {}".format(resource_group),
            checks=[JMESPathCheck("length([?name=='{}' && resourceGroup=='{}'])".format(plan, resource_group), 1)],
        )
        self.cmd("appservice plan show -g {} -n {}".format(resource_group, plan), checks=[JMESPathCheck("name", plan)])
        self.cmd(
        ...
        self.cmd(
            "webapp list -g {}".format(resource_group),
            checks=[
                JMESPathCheck("length(@)", 1),
        ...
        self.cmd(
            "webapp show -g {} -n {}".format(resource_group, webapp_name),
            checks=[
                JMESPathCheck("name", webapp_name),
                JMESPathCheck("hostNames[0]", webapp_name + ".azurewebsites.net"),
            ],
        ...
        result = self.cmd(
            "webapp deployment source config-local-git -g {} -n {}".format(resource_group, webapp_name)
        ).get_output_in_json()
        self.assertTrue(result["url"].endswith(webapp_name + ".git"))
        self.cmd(
            "webapp deployment source show -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("repoUrl", "https://{}.scm.azurewebsites.net".format(webapp_name))],
        )
        # turn on diagnostics
        test_cmd = (
            "webapp log config -g {} -n {} --level verbose".format(resource_group, webapp_name) + " "
            "--application-logging filesystem --detailed-error-messages true --failed-request-tracing true --web-server-logging filesystem"
        )
        self.cmd(test_cmd)
        self.cmd(
            "webapp log show -g {} -n {}".format(resource_group, webapp_name),
            checks=[
        ...
        self.cmd(
            "webapp config show -g {} -n {}".format(resource_group, webapp_name),
            checks=[
        ...
        result = self.cmd(
            "webapp deployment list-publishing-profiles -g {} -n {}".format(resource_group, webapp_name)
        ).get_output_in_json()
        self.assertTrue(result[1]["publishUrl"].startswith("ftp://"))
        self.cmd("webapp stop -g {} -n {}".format(resource_group, webapp_name))
        self.cmd(
            "webapp show -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("state", "Stopped"), JMESPathCheck("name", webapp_name)],
        )
        self.cmd("webapp start -g {} -n {}".format(resource_group, webapp_name))
        self.cmd(
            "webapp show -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("state", "Running"), JMESPathCheck("name", webapp_name)],
        )
        # show publishing credentials
        result = self.cmd(
            "webapp deployment list-publishing-credentials -g {} -n {}".format(resource_group, webapp_name)
        ).get_output_in_json()
        ...
        self.cmd('webapp create -g {} -n {} --plan {} -r "php|7.4"'.format(resource_group, webapp_name, plan))

        self.cmd(
            "webapp config show -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("phpVersion", "7.4")],
        )
...
class WebappQuickCreateTest(ScenarioTest):
    ...
    def test_win_webapp_quick_create(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {}".format(resource_group, plan))
        r = self.cmd(
            "webapp create -g {} -n {} --plan {} --deployment-local-git".format(resource_group, webapp_name, plan)
        ).get_output_in_json()
        self.assertTrue(r["ftpPublishingUrl"].startswith("ftp://"))
        self.cmd(
            "webapp config appsettings list -g {} -n {}".format(resource_group, webapp_name),
            checks=[
    ...
    def test_win_webapp_quick_create_runtime(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {}".format(resource_group, plan))
        r = self.cmd(
            'webapp create -g {} -n {} --plan {} --deployment-local-git -r "node|14LTS"'.format(
        ...
        r = self.cmd(
            'webapp create -g {} -n {} --plan {} --deployment-local-git -r "DOTNETCORE|3.1"'.format(
                resource_group, webapp_name_2, plan
            )
        ).get_output_in_json()
        self.assertTrue(r["ftpPublishingUrl"].startswith("ftp://"))
    ...
    def test_win_webapp_quick_create_cd(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {}".format(resource_group, plan))
        self.cmd(
            'webapp create -g {} -n {} --plan {} --deployment-source-url {} -r "node|12LTS"'.format(
    ...
    def test_linux_webapp_multicontainer_create(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {} --is-linux".format(resource_group, plan))
        self.cmd(
            'webapp create -g {} -n {} --plan {} --multicontainer-config-file "{}" '
    ...
    def test_linux_webapp_quick_create_cd(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {} --is-linux".format(resource_group, plan))
        self.cmd(
            'webapp create -g {} -n {} --plan {} -u {} -r "NODE|14-lts"'.format(
                resource_group, webapp_name, plan, TEST_REPO_URL
            )
        )
        # 45 seconds should be enough for the deployment finished(Skipped under playback mode)
        time.sleep(45)
        r = requests.get("http://{}.azurewebsites.net".format(webapp_name), timeout=500)
        # verify the web page
    ...
    @ResourceGroupPreparer(
        parameter_name="resource_group2",
        parameter_name_for_location="resource_group_location2",
        location=WINDOWS_ASP_LOCATION_WEBAPP,
    )
    def test_create_in_different_group(
        self, resource_group, resource_group_location, resource_group2, resource_group_location2
    ):
        ...
        self.cmd(
            "webapp create -g {} -n {} --plan {}".format(resource_group2, webapp_name, plan_id),
            checks=[JMESPathCheck("name", webapp_name)],
        )

    @unittest.skip("This test needs to be updated first to have unique names & re-tested before recording")
    @AllowLargeResponse()
    @ResourceGroupPreparer(
        parameter_name="resource_group_one",
        name_prefix="clitest",
        random_name_length=24,
        location=WINDOWS_ASP_LOCATION_WEBAPP,
    )
    @ResourceGroupPreparer(
        parameter_name="resource_group_two",
        name_prefix="clitest",
        random_name_length=24,
        location=WINDOWS_ASP_LOCATION_WEBAPP,
    )
    def test_create_names_are_substrings(self, resource_group_one, resource_group_two):
        ...
        plan_id_one = self.cmd(
            "appservice plan create -g {} -n {}".format(resource_group_one, plan_name_one)
        ).get_output_in_json()["id"]
        plan_id_two = self.cmd(
            "appservice plan create -g {} -n {}".format(resource_group_two, plan_name_two)
        ).get_output_in_json()["id"]
        self.cmd(
            "webapp create -g {} -n {} --plan {}".format(resource_group_one, webapp_name_one, plan_id_one),
            checks=[JMESPathCheck("name", webapp_name_one)],
        )
        self.cmd(
            "webapp create -g {} -n {} --plan {}".format(resource_group_two, webapp_name_two, plan_id_two),
            checks=[JMESPathCheck("name", webapp_name_two)],
        )
        self.cmd(
            "webapp create -g {} -n {} --plan {}".format(resource_group_one, webapp_name_three, plan_id_one),
            checks=[JMESPathCheck("name", webapp_name_three)],
        )
...
class BackupWithName(ScenarioTest):
    ...
    def test_backup_with_name(self, resource_group):
        ...
        self.cmd(
            "storage account create -n {} -g {} --location {}".format(
                storage_Account, resource_group, WINDOWS_ASP_LOCATION_WEBAPP
            )
        )
        ...
        sastoken = self.cmd(
            "storage container generate-sas --account-name {} --name {} --expiry {} --permissions rwdl".format(
                storage_Account, container, expirydate
            )
        )
        ...
        self.cmd(
            "webapp config backup create -g {} --webapp-name {} --backup-name {} --container-url {}".format(
                resource_group, webapp, backup_name, sasurl
            ),
            checks=[JMESPathCheck("blobName", backup_name)],
        )
        self.cmd(
            "webapp config backup list -g {} --webapp-name {}".format(resource_group, webapp),
            checks=[JMESPathCheck("length(@)", 1), JMESPathCheck("[0].namePropertiesName", backup_name)],
        )
...
class AppServicePlanScenarioTest(ScenarioTest):
    ...
    def test_elastic_scale_plan(self, resource_group):
        ...
        self.cmd(
            "appservice plan show -g {} -n {}".format(resource_group, plan),
            checks=[JMESPathCheck("properties.elasticScaleEnabled", False)],
        )

        self.cmd("appservice plan update -g {} -n {} --elastic-scale true".format(resource_group, plan))
        self.cmd(
            "appservice plan show -g {} -n {}".format(resource_group, plan),
            checks=[JMESPathCheck("properties.elasticScaleEnabled", True)],
        )
        ...
        self.cmd(
            "appservice plan update -g {} -n {} --elastic-scale true".format(resource_group, plan), expect_failure=True
        )
    ...
    def test_elastic_scale_plan_max_workers(self, resource_group):
        ...
        self.cmd(
            "appservice plan update -g {} -n {} --elastic-scale true --max-elastic-worker-count 10".format(
                resource_group, plan
            )
        )
        self.cmd(
        ...
        self.cmd(
            "appservice plan update -g {} -n {} --max-elastic-worker-count 9 --number-of-workers 10".format(
                resource_group, plan
            ),
            expect_failure=True,
        )
        ...
        self.cmd(
            "appservice plan update -g {} -n {} --max-elastic-worker-count 10 --number-of-workers 10".format(
                resource_group, plan
            )
        )
        self.cmd(
            "appservice plan update -g {} -n {} --max-elastic-worker-count 9".format(resource_group, plan),
            expect_failure=True,
        )
    ...
    def test_retain_plan(self, resource_group):
        ...
        self.cmd(
            "webapp delete -g {} -n {} --keep-dns-registration --keep-empty-plan --keep-metrics".format(
                resource_group, webapp_name
            )
        )
        self.cmd("appservice plan list -g {}".format(resource_group), checks=[JMESPathCheck("[0].name", plan)])
    ...
    def test_auto_delete_plan(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {} -l {}".format(resource_group, plan, WINDOWS_ASP_LOCATION_WEBAPP))
    ...
    def test_zone_redundant_plan(self, resource_group):
        plan = self.create_random_name("plan", 24)
        self.cmd(
            "appservice plan create -g {} -n {} -l {} -z --sku P1V3".format(
                resource_group, plan, LINUX_ASP_LOCATION_WEBAPP
            )
        )

        self.cmd(
            "appservice plan show -g {} -n {}".format(resource_group, plan),
            checks=[JMESPathCheck("properties.zoneRedundant", True), JMESPathCheck("properties.numberOfWorkers", 3)],
        )

        # test with unsupported SKU
        plan2 = self.create_random_name("plan", 24)
        self.cmd(
            "appservice plan create -g {} -n {} -l {} -z --sku FREE".format(
                resource_group, plan2, LINUX_ASP_LOCATION_WEBAPP
            ),
            expect_failure=True,
        )
        ...
        self.cmd(
            "appservice plan create -g {} -n {} -l {} -z --sku P1V3".format(
                resource_group, plan2, WINDOWS_ASP_LOCATION_WEBAPP
            ),
            expect_failure=True,
        )
        ...
        plan = self.create_random_name("plan", 24)
        self.cmd(
            "functionapp plan create -g {} -n {} -l {} --sku FREE".format(
                resource_group, plan, LINUX_ASP_LOCATION_WEBAPP
            )
        )
        ...
        self.cmd(
            "appservice plan show -g {} -n {}".format(resource_group, plan),
            checks=[JMESPathCheck("properties.zoneRedundant", False)],
        )
...
class WebappElasticScaleTest(ScenarioTest):
    ...
    def test_webapp_elastic_scale(self, resource_group):
        ...
        self.cmd(
            "webapp show -g {} -n {}".format(resource_group, app),
            checks=[
        ...
        self.cmd(
            "webapp update -g {} -n {} --minimum-elastic-instance-count {} --prewarmed-instance-count {}".format(
                resource_group, app, 3, 5
            )
        )
    ...
    def test_webapp_elastic_scale_prewarmed_instance_count(self, resource_group):
        ...
        self.cmd(
            "webapp update -g {} -n {} --prewarmed-instance-count {}".format(resource_group, app, 5),
            expect_failure=True,
        )

        self.cmd(
            "appservice plan update -g {} -n {} --elastic-scale true --max-elastic-worker-count {}".format(
                resource_group, plan, 4
            )
        )
        self.cmd(
            "appservice plan show -g {} -n {}".format(resource_group, plan),
        ...
        self.cmd(
            "webapp show -g {} -n {}".format(resource_group, app),
            checks=[JMESPathCheck("siteConfig.preWarmedInstanceCount", 4)],
        )
    ...
    def test_webapp_elastic_scale_min_elastic_instance_count(self, resource_group):
        ...
        # ensure we can't set this without the ASP having elastic scale enabled
        self.cmd(
            "webapp update -g {} -n {} --minimum-elastic-instance-count {}".format(resource_group, app, 5),
            expect_failure=True,
        )
        ...
        self.cmd(
            "webapp show -g {} -n {}".format(resource_group, app),
            checks=[JMESPathCheck("siteConfig.minimumElasticInstanceCount", 4)],
        )
...
class WebappConfigureTest(ScenarioTest):
    ...
    def test_webapp_config(self, resource_group):
        ...
        self.cmd(
            "webapp config set -g {} -n {} --always-on true --auto-heal-enabled true --php-version 7.2 "
            "--net-framework-version v3.5 --python-version 3.4 --use-32bit-worker-process=false "
            "--web-sockets-enabled=true --http20-enabled true --min-tls-version 1.0 --ftps-state Disabled".format(
                resource_group, webapp_name
            )
        ).assert_with_checks(checks)
        self.cmd("webapp config show -g {} -n {}".format(resource_group, webapp_name)).assert_with_checks(checks)

        # site appsettings testing
        # update through key value pairs
        self.cmd(
            "webapp config appsettings set -g {} -n {} --settings s1=foo s2=bar s3=bar2".format(
                resource_group, webapp_name
            )
        ).assert_with_checks(
            [
        ...
        # delete
        self.cmd(
            "webapp config appsettings delete -g {} -n {} --setting-names s1 s2".format(resource_group, webapp_name)
        ).assert_with_checks(
            [
                JMESPathCheck("length([?name=='s3'])", 1),
        ...
        # hostnames
        self.cmd(
            "webapp config hostname list -g {} --webapp-name {}".format(resource_group, webapp_name)
        ).assert_with_checks(
            [JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", "{0}.azurewebsites.net".format(webapp_name))]
        )
        ...
        self.cmd(
            "appservice plan create -g {} -n {} -l eastus2 --sku S1 --is-linux".format(resource_group, linux_plan),
            checks=[
        ...
        self.cmd(
            "webapp create -g {} -n {} --plan {} --runtime {}".format(
                resource_group, linux_webapp, linux_plan, runtime
            ),
            checks=[
        ...
        self.cmd(
            (
                "webapp config storage-account add -g {} -n {} --custom-id Id --storage-type AzureFiles --account-name name "
                "--share-name sharename --access-key key --mount-path /path/to/mount"
            ).format(resource_group, linux_webapp)
        )
        self.cmd(
            "webapp config storage-account list -g {} -n {}".format(resource_group, linux_webapp)
        ).assert_with_checks(
            [
                JMESPathCheck("length(@)", 1),
        ...
        # update
        self.cmd(
            "webapp config storage-account update -g {} -n {} --custom-id Id --mount-path /different/path".format(
                resource_group, linux_webapp
            )
        )
        self.cmd(
            "webapp config storage-account list -g {} -n {}".format(resource_group, linux_webapp)
        ...
        self.cmd(
            "webapp config storage-account list -g {} -n {}".format(resource_group, linux_webapp)
        ).assert_with_checks([JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", "Id")])
        # delete
        self.cmd(
            "webapp config storage-account delete -g {} -n {} --custom-id Id".format(resource_group, linux_webapp)
        ).assert_with_checks([JMESPathCheck("length(@)", 0)])

        # site connection string tests
        self.cmd(
            'webapp config connection-string set -t mysql -g {} -n {} --settings c1="conn1" c2=conn2 '
            "--slot-settings c3=conn3".format(resource_group, linux_webapp)
        )
        self.cmd(
            "webapp config connection-string list -g {} -n {}".format(resource_group, linux_webapp)
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "webapp config connection-string delete -g {} -n {} --setting-names c1 c3".format(
                resource_group, linux_webapp
            )
        )
        self.cmd(
            ...
            [JMESPathCheck("length([])", 1), JMESPathCheck("[0].slotSetting", False), JMESPathCheck("[0].name", "c2")]
        )
    ...
    @ResourceGroupPreparer(
        name_prefix="cli_test_webapp_update_site_configs_persists_ip_restrictions", location=WINDOWS_ASP_LOCATION_WEBAPP
    )
    def test_webapp_update_site_configs_persists_ip_restrictions(self, resource_group):
        ...
        self.cmd(
            "network vnet create -g {} -n {} --address-prefix 10.0.0.0/16 --subnet-name {} --subnet-prefix 10.0.0.0/24".format(
        ...
        self.cmd(
            "webapp config set -g {} -n {} --always-on true".format(resource_group, webapp_name)
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "webapp config access-restriction add -g {} -n {} --rule-name testclirule --priority 300 --subnet {} --vnet-name {}".format(
    ...
    def test_update_webapp_settings_thru_json(self, resource_group):
        ...
        self.cmd(
            "webapp deployment slot create -g {} -n {} --slot {}".format(resource_group, webapp_name, slot),
            checks=[JMESPathCheck("name", slot)],
        )

        output = self.cmd(
            'webapp config appsettings set -g {} -n {} --settings s=value "@{}"'.format(
                resource_group, webapp_name, settings_file
            )
        ).get_output_in_json()
        output = [s for s in output if s["name"] in ["s", "s2"]]
        output.sort(key=lambda s: s["name"])
        self.assertEqual(output[0], {"name": "s", "value": "value", "slotSetting": False})
        self.assertEqual(output[1], {"name": "s2", "value": "value2", "slotSetting": False})

        # output using the output of the set/list command
        output.append({"name": "s3", "value": "value3", "slotSetting": True})
        with open(settings_file, "w") as file:
            file.write(json.dumps(output))

        output = self.cmd(
            'webapp config appsettings set -g {} -n {} --settings "@{}"'.format(
                resource_group, webapp_name, settings_file
            )
        ).get_output_in_json()
        ...
        self.assertEqual(output[0], {"name": "s", "value": "value", "slotSetting": False})
        self.assertEqual(output[1], {"name": "s2", "value": "value2", "slotSetting": False})
        self.assertEqual(output[2], {"name": "s3", "value": "value3", "slotSetting": True})
        with open(settings_file, "w") as file:
            file.write(json.dumps(output))

        output = self.cmd(
            'webapp config appsettings set -g {} -n {} --slot {} --settings "@{}"'.format(
        ...
        self.assertEqual(output[2], {"name": "s3", "value": "value3", "slotSetting": True})
        # update site config
        site_configs = {"requestTracingEnabled": True, "alwaysOn": True}
        with open(settings_file, "w") as file:
            file.write(json.dumps(site_configs))
        self.cmd(
            'webapp config set -g {} -n {} --generic-configurations "@{}"'.format(
                resource_group, webapp_name, settings_file
            )
        ).assert_with_checks(
            [
...
class LinuxWebappScenarioTest(ScenarioTest):
    ...
    def test_linux_webapp(self, resource_group):
        ...
        self.cmd(
            "webapp create -g {} -n {} --plan {} --runtime {}".format(resource_group, webapp, plan, runtime),
            checks=[
                JMESPathCheck("name", webapp),
            ],
        )
        self.cmd(
            "webapp config show -g {} -n {}".format(resource_group, webapp),
            checks=[JMESPathCheck("windowsFxVersion", None)],
        )
        ...
        self.cmd(
            "webapp show -g {} -n {}".format(resource_group, webapp),
            checks=[JMESPathCheck("name", webapp), JMESPathCheck("kind", "app,linux")],
        )
        self.cmd(
            "webapp config set -g {} -n {} --startup-file {}".format(resource_group, webapp, "process.json"),
            checks=[JMESPathCheck("appCommandLine", "process.json")],
        )

        result = self.cmd(
            "webapp deployment container config -g {} -n {} --enable-cd true".format(resource_group, webapp)
        ).get_output_in_json()
        ...
        result = self.cmd(
            "webapp config container set -g {} -n {} --docker-custom-image-name {} --docker-registry-server-password {} --docker-registry-server-user {} --docker-registry-server-url {} --enable-app-service-storage {}".format(
                resource_group, webapp, "foo-image", "foo-password", "foo-user", "foo-url", "false"
            )
        ).get_output_in_json()
        self.assertEqual(
            set(x["value"] for x in result if x["name"] == "DOCKER_REGISTRY_SERVER_PASSWORD"), set([None])
        )  # we mask the password

        result = self.cmd(
            "webapp config container show -g {} -n {} ".format(resource_group, webapp)
        ).get_output_in_json()
        self.assertEqual(
        ...
        self.assertEqual(
            sample, {"name": "WEBSITES_ENABLE_APP_SERVICE_STORAGE", "slotSetting": False, "value": "false"}
        )
...
class LinuxWebappMulticontainerSlotScenarioTest(ScenarioTest):
    ...
    def test_linux_webapp_multicontainer_slot(self, resource_group):
        ...
        self.cmd(
            'webapp config container set -g {} -n {} --slot {} --multicontainer-config-file "{}" '
...
class WebappACRScenarioTest(ScenarioTest):
    ...
    def test_acr_integration(self, resource_group):
        ...
        self.cmd("webapp create -g {} -n {} --plan {} --runtime {}".format(resource_group, webapp, plan, runtime))
        creds = self.cmd(
            "acr credential show -n {} -g {}".format(acr_registry_name, resource_group)
        ).get_output_in_json()
        self.cmd(
            "webapp config container set -g {0} -n {1} --docker-custom-image-name {2}.azurecr.io/image-name:latest --docker-registry-server-url https://{2}.azurecr.io".format(
                resource_group, webapp, acr_registry_name
            ),
            checks=[JMESPathCheck("[?name=='DOCKER_REGISTRY_SERVER_USERNAME']|[0].value", creds["username"])],
        )
...
class WebappGitScenarioTest(ScenarioTest):
    ...
    def test_webapp_git(self, resource_group):
        ...
        self.cmd("webapp create -g {} -n {} --plan {}".format(resource_group, webapp, plan))
        self.cmd(
            "webapp deployment source config -g {} -n {} --repo-url {} --branch {} --manual-integration".format(
                resource_group, webapp, test_git_repo, "master"
            ),
            checks=[
        ...
        self.cmd(
            "webapp deployment source show -g {} -n {}".format(resource_group, webapp),
            checks=[
                JMESPathCheck("repoUrl", test_git_repo),
                JMESPathCheck("isMercurial", False),
                JMESPathCheck("branch", "master"),
...
class WebappSlotScenarioTest(ScenarioTest):
    ...
    def test_webapp_slot(self, resource_group):
        ...
        self.cmd(
            "webapp config appsettings set -g {} -n {} --settings s1=v1 --slot-settings s2=v2".format(
        ...
        self.cmd(
            "webapp deployment source config -g {} -n {} --repo-url {} --branch {} -s {} --manual-integration".format(
                resource_group, webapp, test_git_repo, slot, slot
            ),
            checks=[JMESPathCheck("repoUrl", test_git_repo), JMESPathCheck("branch", slot)],
        )
        ...
        result = self.cmd(
            "webapp config appsettings list -g {} -n {} -s {}".format(resource_group, webapp, slot)
        ).get_output_in_json()
        ...
        self.cmd(
            "webapp deployment slot create -g {} -n {} --slot {} --configuration-source {}".format(
                resource_group, webapp, slot2, webapp
            )
        )
        self.cmd(
            "webapp config show -g {} -n {} --slot {}".format(resource_group, webapp, slot2),
            checks=[
                JMESPathCheck("phpVersion", test_php_version),
            ],
        )
        self.cmd(
            "webapp config appsettings set -g {} -n {} --slot {} --settings s3=v3 --slot-settings s4=v4".format(
                resource_group, webapp, slot2
            ),
            checks=[
        ...
        self.cmd(
            "webapp config connection-string set -g {} -n {} -t mysql --slot {} --settings c1=connection1 --slot-settings c2=connection2".format(
                resource_group, webapp, slot2
            )
        )
        # verify we can swap with non production slot
        self.cmd(
            "webapp deployment slot swap -g {} -n {} --slot {} --target-slot {}".format(
                resource_group, webapp, slot, slot2
            )
        )
        result = self.cmd(
            "webapp config appsettings list -g {} -n {} --slot {}".format(resource_group, webapp, slot2)
        ).get_output_in_json()
        self.assertEqual(set([x["name"] for x in result]), set(["s1", "s4", "WEBSITE_NODE_DEFAULT_VERSION"]))
        result = self.cmd(
            "webapp config connection-string list -g {} -n {} --slot {}".format(resource_group, webapp, slot2)
        ).get_output_in_json()
        self.assertEqual(set([x["name"] for x in result]), set(["c2"]))
        result = self.cmd(
            "webapp config appsettings list -g {} -n {} --slot {}".format(resource_group, webapp, slot)
        ).get_output_in_json()
        self.assertTrue(set(["s3"]).issubset(set([x["name"] for x in result])))
        result = self.cmd(
            "webapp config connection-string list -g {} -n {} --slot {}".format(resource_group, webapp, slot)
        ).get_output_in_json()
        self.assertEqual(set([x["name"] for x in result]), set(["c1"]))
        self.cmd(
            "webapp deployment slot list -g {} -n {}".format(resource_group, webapp),
            checks=[
        ...
        self.cmd(
            "webapp deployment slot auto-swap -g {} -n {} -s {}".format(resource_group, webapp, slot),
            checks=[JMESPathCheck("autoSwapSlotName", "production")],
        )
        self.cmd(
            "webapp deployment slot auto-swap -g {} -n {} -s {} --disable".format(resource_group, webapp, slot),
            checks=[JMESPathCheck("autoSwapSlotName", None)],
        )
        ...
        self.cmd(
            "webapp delete -g {} -n {} --slot {} --keep-dns-registration --keep-empty-plan --keep-metrics".format(
                resource_group, webapp, slot2
            )
        )
...
class WebappSlotTrafficRouting(ScenarioTest):
    ...
    def test_traffic_routing(self, resource_group):
        ...
        self.cmd(
            "webapp traffic-routing set -g {} -n {} -d {}=15".format(resource_group, webapp, slot),
            checks=[
        ...
        self.cmd(
            "webapp traffic-routing show -g {} -n {}".format(resource_group, webapp),
            checks=[
                JMESPathCheck("[0].actionHostName", webapp + "-" + slot + ".azurewebsites.net"),
                JMESPathCheck("[0].reroutePercentage", 15.0),
            ],
...
class AppServiceCors(ScenarioTest):
    ...
    def test_webapp_cors(self, resource_group):
        ...
        self.cmd(
            "webapp cors show -g {rg} -n {web} --slot {slot}", checks=self.check("allowedOrigins", ["https://foo.com"])
        )
...
class WebappSlotSwapScenarioTest(ScenarioTest):
    ...
    def test_webapp_slot_swap(self, resource_group):
        ...
        self.cmd("webapp deployment slot create -g {} -n {} --slot {}".format(resource_group, webapp, slot))
        self.cmd(
            "webapp config appsettings set -g {} -n {} --slot-settings s1=slot --slot {}".format(
        ...
        self.cmd(
            "webapp config appsettings list -g {} -n {} --slot {}".format(resource_group, webapp, slot),
            checks=[JMESPathCheck("[?name=='s1']|[0].value", "prod")],
        )
        # complete the swap
        self.cmd("webapp deployment slot swap -g {} -n {} -s {}".format(resource_group, webapp, slot))
        self.cmd(
            "webapp config appsettings list -g {} -n {} --slot {}".format(resource_group, webapp, slot),
            checks=[JMESPathCheck("[?name=='s1']|[0].value", "slot")],
        )
        # reset
...
class WebappSSLCertTest(ScenarioTest):
    ...
    def test_webapp_ssl(self, resource_group, resource_group_location):
        ...
        self.cmd("webapp create -g {} -n {} --plan {} --tags web=web1".format(resource_group, webapp_name, plan))
        self.cmd(
            'webapp config ssl upload -g {} -n {} --certificate-file "{}" --certificate-password {}'.format(
                resource_group, webapp_name, pfx_file, cert_password
            ),
        ...
        self.cmd(
            "webapp config ssl bind -g {} -n {} --certificate-thumbprint {} --ssl-type {}".format(
                resource_group, webapp_name, cert_thumbprint, "SNI"
            ),
            checks=[
                JMESPathCheck(
                    "hostNameSslStates|[?name=='{}.azurewebsites.net']|[0].sslState".format(webapp_name), "SniEnabled"
                ),
                JMESPathCheck(
                    "hostNameSslStates|[?name=='{}.azurewebsites.net']|[0].thumbprint".format(webapp_name),
                    cert_thumbprint,
                ),
            ],
        )
        self.cmd("webapp show -g {} -n {}".format(resource_group, webapp_name), self.check("tags.web", "web1"))
        self.cmd(
            "webapp config ssl unbind -g {} -n {} --certificate-thumbprint {}".format(
                resource_group, webapp_name, cert_thumbprint
            ),
            checks=[
                JMESPathCheck(
                    "hostNameSslStates|[?name=='{}.azurewebsites.net']|[0].sslState".format(webapp_name), "Disabled"
                ),
            ],
        )
        self.cmd("webapp show -g {} -n {}".format(resource_group, webapp_name), self.check("tags.web", "web1"))
        ...
        self.cmd(
            'webapp config ssl upload -g {} -n {} --certificate-file "{}" --certificate-password {} -s {}'.format(
                resource_group, webapp_name, pfx_file, cert_password, slot_name
            ),
            checks=[JMESPathCheck("thumbprint", cert_thumbprint)],
        )
        self.cmd("webapp show -g {} -n {} -s {}".format(resource_group, webapp_name, slot_name))
        self.cmd(
            "webapp config ssl bind -g {} -n {} --certificate-thumbprint {} --ssl-type {} -s {}".format(
                resource_group, webapp_name, cert_thumbprint, "SNI", slot_name
            ),
            checks=[
                JMESPathCheck(
                    "hostNameSslStates|[?name=='{}-{}.azurewebsites.net']|[0].sslState".format(webapp_name, slot_name),
                    "SniEnabled",
                ),
                JMESPathCheck(
                    "hostNameSslStates|[?name=='{}-{}.azurewebsites.net']|[0].thumbprint".format(
                        webapp_name, slot_name
                    ),
                    cert_thumbprint,
                ),
            ],
        ...
        self.cmd("webapp show -g {} -n {} -s {}".format(resource_group, webapp_name, slot_name))
        self.cmd(
            "webapp config ssl unbind -g {} -n {} --certificate-thumbprint {} -s {}".format(
                resource_group, webapp_name, cert_thumbprint, slot_name
            ),
            checks=[
                JMESPathCheck(
                    "hostNameSslStates|[?name=='{}-{}.azurewebsites.net']|[0].sslState".format(webapp_name, slot_name),
...
class WebappSSLImportCertTest(ScenarioTest):
    ...
    def test_webapp_ssl_import(self, resource_group, key_vault):
        ...
        self.cmd("webapp create -g {} -n {} --plan {}".format(resource_group, webapp_name, plan_name))
        self.cmd(
            "keyvault set-policy -g {} --name {} --spn {} --secret-permissions get".format(
                resource_group, key_vault, "Microsoft.Azure.WebSites"
            )
        )
        self.cmd(
            'keyvault certificate import --name {} --vault-name {} --file "{}" --password {}'.format(
                cert_name, key_vault, pfx_file, cert_password
            )
        )

        self.cmd(
            "webapp config ssl import --resource-group {} --name {}  --key-vault {} --key-vault-certificate-name {}".format(
                resource_group, webapp_name, key_vault, cert_name
            ),
            checks=[JMESPathCheck("thumbprint", cert_thumbprint)],
        )
    ...
    def test_webapp_ssl_import_crossrg(self, resource_group, kv_resource_group):
        ...
        self.cmd(
            "webapp config ssl import --resource-group {} --name {}  --key-vault {} --key-vault-certificate-name {}".format(
                resource_group, webapp_name, kv_id, cert_name
            ),
            checks=[JMESPathCheck("thumbprint", cert_thumbprint)],
        )
...
class WebappUndeleteTest(ScenarioTest):
    ...
    def test_webapp_deleted_list(self, resource_group):
        ...
        self.cmd("webapp delete -g {} -n {}".format(resource_group, webapp_name))
        self.cmd(
            "webapp deleted list -g {}".format(resource_group),
            checks=[JMESPathCheck("[0].deletedSiteName", webapp_name)],
        )
...
class WebappAuthenticationTest(ScenarioTest):
    ...
    def test_webapp_authentication(self, resource_group):
        ...
        result = (
            self.cmd(
                "webapp auth update -g {} -n {} --enabled true --action LoginWithFacebook "
                ...
                "--facebook-oauth-scopes public_profile email".format(resource_group, webapp_name)
            )
            .assert_with_checks(
                [
...
class WebappUpdateTest(ScenarioTest):
    ...
    def test_webapp_update(self, resource_group):
        ...
        ).assert_with_checks([JMESPathCheck("clientAffinityEnabled", True)])
        # testing update command with --set
        self.cmd(
            "webapp update -g {} -n {} --client-affinity-enabled false --set tags.foo=bar".format(
                resource_group, webapp_name
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "webapp update -g {} -n {} -s s1 --client-affinity-enabled true".format(resource_group, webapp_name),
...
class WebappZipDeployScenarioTest(ScenarioTest):
    ...
    def test_deploy_zip(self, resource_group):
        ...
        self.cmd("webapp create -g {} -n {} --plan {}".format(resource_group, webapp_name, plan_name))
        self.cmd(
            'webapp deployment source config-zip -g {} -n {} --src "{}"'.format(resource_group, webapp_name, zip_file)
        ).assert_with_checks(
            [
...
class WebappImplictIdentityTest(ScenarioTest):
    ...
    def test_webapp_assign_system_identity(self, resource_group):
        ...
        with mock.patch("azure.cli.core.commands.arm._gen_guid", side_effect=self.create_guid):
            result = self.cmd(
                "webapp identity assign -g {} -n {} --role {} --scope {}".format(
                    resource_group, webapp_name, role, scope
                )
            ).get_output_in_json()
            self.cmd(
                "webapp identity show -g {} -n {}".format(resource_group, webapp_name),
                checks=[self.check("principalId", result["principalId"])],
            )

        self.cmd(
            "role assignment list -g {} --assignee {}".format(resource_group, result["principalId"]),
            checks=[JMESPathCheck("length([])", 1), JMESPathCheck("[0].roleDefinitionName", role)],
        )
        self.cmd(
            "webapp identity show -g {} -n {}".format(resource_group, webapp_name),
            checks=self.check("principalId", result["principalId"]),
    ...
    def test_webapp_assign_user_identity(self, resource_group):
        ...
        msi_result = self.cmd(
            "identity create -g {} -n {}".format(resource_group, identity_name),
            checks=[self.check("name", identity_name)],
        ).get_output_in_json()
        self.cmd("appservice plan create -g {} -n {}".format(resource_group, plan_name))
        self.cmd("webapp create -g {} -n {} --plan {}".format(resource_group, webapp_name, plan_name))
        ...
        result = self.cmd(
            "webapp identity assign -g {} -n {} --identities {}".format(resource_group, webapp_name, msi_result["id"])
        ).get_output_in_json()
        self.cmd(
            "webapp identity show -g {} -n {}".format(resource_group, webapp_name),
            checks=[
        ...
        self.cmd(
            "webapp identity remove -g {} -n {} --identities {}".format(resource_group, webapp_name, msi_result["id"])
        )
        self.cmd(
            "webapp identity show -g {} -n {}".format(resource_group, webapp_name),
            checks=[
    ...
    def test_webapp_remove_identity(self, resource_group):
        ...
        msi2_result = self.cmd(
            "identity create -g {} -n {}".format(resource_group, identity2_name)
        ).get_output_in_json()
        self.cmd("appservice plan create -g {} -n {}".format(resource_group, plan_name))
        self.cmd("webapp create -g {} -n {} --plan {}".format(resource_group, webapp_name, plan_name))

        self.cmd(
            "webapp identity assign -g {} -n {} --identities [system] {} {}".format(
                resource_group, webapp_name, msi_result["id"], msi2_result["id"]
            )
        )

        result = self.cmd(
            "webapp identity remove -g {} -n {} --identities {}".format(resource_group, webapp_name, msi2_result["id"])
        ).get_output_in_json()
        self.cmd(
            "webapp identity show -g {} -n {}".format(resource_group, webapp_name),
            checks=[
        ...
        self.cmd(
            "webapp identity remove -g {} -n {} --identities [system] {}".format(
...
class ContainerWebappE2ETest(ScenarioTest):
    ...
    def test_container_webapp_long_server_url(self, resource_group):
        ...
        self.cmd(
            "webapp create -n {} -g {} -p {} -i {}".format(webapp_name, resource_group, plan, container),
            checks=[
                JMESPathCheck("state", "Running"),
                JMESPathCheck("name", webapp_name),
                JMESPathCheck("hostNames[0]", webapp_name + ".azurewebsites.net"),
...
class WebappWindowsContainerBasicE2ETest(ScenarioTest):
    ...
    def test_webapp_hyperv_e2e(self, resource_group):
        ...
        self.cmd("appservice plan show -g {} -n {}".format(resource_group, plan), checks=[JMESPathCheck("name", plan)])
        self.cmd(
            'webapp create -g {} -n {} --plan {} --deployment-container-image-name "DOCKER|microsoft/iis:windowsservercore-ltsc2022"'.format(
                resource_group, webapp_name, plan
            ),
            checks=[
                JMESPathCheck("state", "Running"),
        ...
        self.cmd(
            "webapp config show -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("windowsFxVersion", "DOCKER|microsoft/iis"), JMESPathCheck("linuxFxVersion", "")],
        )
        self.cmd(
            'webapp config set -g {} -n {} --windows-fx-version "DOCKER|microsoft/iis"'.format(
                resource_group, webapp_name
            )
        )
        self.cmd(
    ...
    def test_webapp_create_linux_free(self, resource_group):
        ...
        self.cmd(
            "appservice plan create -g {} -n {} --sku F1 --is-linux".format(resource_group, plan),
            checks=[
                # this weird field means it is a linux
                JMESPathCheck("reserved", True),
    ...
    def test_webapp_create_with_msi(self, resource_group):
        ...
        msi_result = self.cmd("identity create -g {} -n {}".format(resource_group, identity_name)).get_output_in_json()
        self.cmd("appservice plan create -g {} -n {}".format(resource_group, plan))
        with mock.patch("azure.cli.core.commands.arm._gen_guid", side_effect=self.create_guid):
            result = self.cmd(
                "webapp create -g {} -n {} --plan {} --assign-identity [system] {} --role {} --scope {}".format(
                    resource_group, webapp_name, plan, msi_result["id"], role, scope
                )
            ).get_output_in_json()
        ...
        self.cmd(
            "role assignment list -g {} --assignee {}".format(resource_group, result["identity"]["principalId"]),
            checks=[self.check("length([])", 1), self.check("[0].roleDefinitionName", role)],
        )
...
class WebappNetworkConnectionTests(ScenarioTest):
    ...
    def test_webapp_hybridconnectionE2E(self, resource_group):
        ...
        self.cmd(
            "relay hyco create -g {} --namespace-name {} --name {} --user-metadata {}".format(
                resource_group, namespace_name, hyco_name, um
            )
        )
        self.cmd(
            "webapp hybrid-connection add -g {} -n {} --namespace {} --hybrid-connection {}".format(
                resource_group, webapp_name, namespace_name, hyco_name
            )
        )
        self.cmd(
            "webapp hybrid-connection list -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", hyco_name)],
        )

        self.cmd(
            "webapp deployment slot create -g {} -n {} --slot {}".format(resource_group, webapp_name, slot_webapp_name)
        )
        self.cmd(
            "webapp hybrid-connection add -g {} -n {} --namespace {} --hybrid-connection {} --slot {}".format(
        ...
        self.cmd(
            "webapp hybrid-connection remove -g {} -n {} --namespace {} --hybrid-connection {}".format(
                resource_group, webapp_name, namespace_name, hyco_name
            )
        )
        self.cmd(
        ...
        self.cmd(
            "webapp hybrid-connection remove -g {} -n {} --namespace {} --hybrid-connection {} --slot {}".format(
                resource_group, webapp_name, namespace_name, hyco_name, slot_webapp_name
            )
        )
        self.cmd(
    ...
    def test_webapp_vnetE2E(self, resource_group):
        ...
        self.cmd("webapp create -g {} -n {} --plan {}".format(resource_group, webapp_name, plan))
        self.cmd(
            "webapp vnet-integration add -g {} -n {} --vnet {} --subnet {}".format(
                resource_group, webapp_name, vnet_name, subnet_name
            )
        )
        self.cmd(
            "webapp vnet-integration list -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", subnet_name)],
        )
        self.cmd(
            "webapp deployment slot create -g {} -n {} --slot {}".format(resource_group, webapp_name, slot_webapp_name)
        )
        self.cmd(
            "webapp vnet-integration add -g {} -n {} --vnet {} --subnet {} --slot {}".format(
                resource_group, webapp_name, vnet_name, subnet_name, slot_webapp_name
            )
        )
        self.cmd(
            "webapp vnet-integration list -g {} -n {} --slot {}".format(resource_group, webapp_name, slot_webapp_name),
            checks=[JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", subnet_name)],
        )
    ...
    def test_webapp_create_vnetE2E(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(resource_group, plan))
        self.cmd(
            "webapp create -g {} -n {} --plan {} --vnet {} --subnet {}".format(
                resource_group, webapp_name, plan, vnet_name, subnet_name
            )
        )
        self.cmd(
            "webapp vnet-integration list -g {} -n {}".format(resource_group, webapp_name),
    ...
    def test_webapp_create_with_vnet_by_subnet_rid(self, webapp_rg, vnet_rg):
        ...
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(webapp_rg, plan))
        self.cmd("webapp create -g {} -n {} --plan {} --subnet {}".format(webapp_rg, webapp_name, plan, subnet_id))
        self.cmd(
            "webapp vnet-integration list -g {} -n {}".format(webapp_rg, webapp_name),
            checks=[JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", subnet_name)],
        )
        self.cmd("webapp vnet-integration remove -g {} -n {}".format(webapp_rg, webapp_name))
        self.cmd(
            "webapp vnet-integration list -g {} -n {}".format(webapp_rg, webapp_name),
            checks=[JMESPathCheck("length(@)", 0)],
        )
    ...
    def test_webapp_create_with_vnet_by_vnet_rid(self, webapp_rg, vnet_rg):
        ...
        ).get_output_in_json()["newVNet"]["id"]
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(webapp_rg, plan))
        self.cmd(
            "webapp create -g {} -n {} --plan {} --vnet {} --subnet {}".format(
                webapp_rg, webapp_name, plan, vnet_id, subnet_name
            )
        )
        self.cmd(
            "webapp vnet-integration list -g {} -n {}".format(webapp_rg, webapp_name),
    ...
    def test_webapp_create_with_vnet_wrong_sku(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {} --sku FREE".format(resource_group, plan))
        self.cmd(
            "webapp create -g {} -n {} --plan {} --vnet {} --subnet {}".format(
                resource_group, webapp_name, plan, vnet_name, subnet_name
            ),
    ...
    def test_webapp_create_with_vnet_wrong_rg(self, webapp_rg, vnet_rg):
        ...
        self.cmd(
            "webapp create -g {} -n {} --plan {} --vnet {} --subnet {}".format(
                webapp_rg, webapp_name, plan, vnet_name, subnet_name
            ),
            expect_failure=True,
        )
    ...
    def test_webapp_create_with_vnet_no_subnet(self, resource_group):
        ...
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(resource_group, plan))
        self.cmd(
            "webapp create -g {} -n {} --plan {} --vnet {}".format(
                resource_group, webapp_name, plan, vnet_name, subnet_name
            ),
            expect_failure=True,
        )
    ...
    def test_webapp_vnetDelegation(self, resource_group):
        ...
        self.cmd(
            "network vnet subnet update -g {} --vnet {} --name {} --delegations Microsoft.Web/serverfarms --service-endpoints Microsoft.Storage".format(
                resource_group, vnet_name, subnet_name
            )
        )
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(resource_group, plan))
        ...
        self.cmd(
            " network vnet subnet show -g {} -n {} --vnet-name {}".format(resource_group, subnet_name, vnet_name),
            checks=[JMESPathCheck("serviceEndpoints[0].service", "Microsoft.Storage")],
        )
        self.cmd("webapp vnet-integration remove -g {} -n {}".format(resource_group, webapp_name))
        self.cmd(
            "webapp vnet-integration list -g {} -n {}".format(resource_group, webapp_name),
    ...
    def test_webapp_vnetSameName(self, resource_group):
        ...
        vnet = self.cmd(
            "network vnet create -g {} -n {} --address-prefix 10.0.0.0/16 --subnet-name {} --subnet-prefix 10.0.0.0/24".format(
                resource_group_2, vnet_name, subnet_name_2
            )
        ).get_output_in_json()
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(resource_group, plan))
        self.cmd("webapp create -g {} -n {} --plan {}".format(resource_group, webapp_name, plan))
        ...
        self.cmd(
            "webapp vnet-integration list -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", subnet_name_2)],
        )
        # self.cmd(
    ...
    def test_webapp_vnetSubnetId(self, resource_group):
        ...
        subnet = self.cmd(
            "network vnet subnet update -g {} --vnet {} --name {} --delegations Microsoft.Web/serverfarms".format(
                resource_group, vnet_name, subnet_name
            )
        ).get_output_in_json()
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(resource_group, plan))
...
class WebappDeploymentLogsScenarioTest(ScenarioTest):
    ...
    def test_webapp_show_deployment_logs(self, resource_group):
        ...
        self.cmd(
            "webapp log deployment show -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("length(@)", 0)],
        )

        deployment_1 = self.cmd(
            'webapp deployment source config-zip -g {} -n {} --src "{}"'.format(resource_group, webapp_name, zip_file)
        ).get_output_in_json()
        self.cmd(
            "webapp log deployment show -g {} -n {}".format(resource_group, webapp_name),
        ...
        self.cmd(
            "webapp log deployment show -g {} -n {} --deployment-id={}".format(
                resource_group, webapp_name, deployment_1["id"]
            ),
            checks=[
                JMESPathCheck("length(@) > `0`", True),
            ],
    ...
    def test_webapp_list_deployment_logs(self, resource_group):
        ...
        self.cmd(
            "webapp log deployment list -g {} -n {}".format(resource_group, webapp_name),
            checks=[JMESPathCheck("length(@)", 0)],
        )
...
class WebappOneDeployScenarioTest(ScenarioTest):
    ...
    def test_one_deploy(self, resource_group):
        ...
        self.cmd(
            'webapp deploy -g {} --n {} --src-path "{}" --type war --async true'.format(
...
class DomainScenarioTest(ScenarioTest):
    ...
    def test_domain_create(self, resource_group):
        ...
        self.cmd(
            "appservice domain create -g {} --hostname {} --contact-info=@'{}' --dryrun".format(
                resource_group, "testuniquedomainname1.com", contacts
            )
        ).assert_with_checks([JMESPathCheck("agreement_keys", "['DNRA', 'DNPA']")])
