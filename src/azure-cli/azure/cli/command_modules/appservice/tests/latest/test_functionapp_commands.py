...
class FunctionappACRScenarioTest(ScenarioTest):
    ...
    def test_acr_integration_function_app(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -s {} --plan {} --functions-version 3 --runtime {}".format(
        ...
        self.cmd(
            "functionapp config container set -g {0} -n {1} --docker-custom-image-name {2}.azurecr.io/image-name:latest --docker-registry-server-url https://{2}.azurecr.io".format(
                resource_group, functionapp, acr_registry_name
            ),
            checks=[JMESPathCheck("[?name=='DOCKER_REGISTRY_SERVER_USERNAME']|[0].value", creds["username"])],
        )
        self.cmd(
            "functionapp config container show -g {} -n {} ".format(resource_group, functionapp),
            checks=[
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp),
            checks=[
...
class FunctionAppCreateUsingACR(ScenarioTest):
    ...
    def test_acr_create_function_app(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -s {} --plan {} --functions-version 3 --runtime {}"
            " --deployment-container-image-name {}.azurecr.io/image-name:latest --docker-registry-server-user {}"
            " --docker-registry-server-password {}".format(
                resource_group, functionapp, storage_account, plan, runtime, acr_registry_name, username, password
            )
        )
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp),
            checks=[
                JMESPathCheck("[?name=='FUNCTIONS_WORKER_RUNTIME'].value|[0]", None),
                JMESPathCheck("[?name=='DOCKER_REGISTRY_SERVER_USERNAME'].value|[0]", username),
            ],
        )

        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[
...
class FunctionappACRDeploymentScenarioTest(ScenarioTest):
    ...
    def test_acr_deployment_function_app(self, resource_group, storage_account):
        ...
        creds = self.cmd(
            "acr credential show -g {} -n {}".format(resource_group, acr_registry_name)
        ).get_output_in_json()
        self.cmd(
            "functionapp config container set -g {0} -n {1} --docker-custom-image-name {2}.azurecr.io/image-name:latest --docker-registry-server-url https://{2}.azurecr.io".format(
                resource_group, functionapp, acr_registry_name
        ...
        result = self.cmd(
            "functionapp deployment container config -g {} -n {} --enable-cd true".format(resource_group, functionapp)
        ).get_output_in_json()
        ...
        show_result = self.cmd(
            "functionapp deployment container show-cd-url -g {} -n {}".format(resource_group, functionapp)
        ).get_output_in_json()
...
class FunctionAppReservedInstanceTest(ScenarioTest):
    ...
    def test_functionapp_reserved_instance(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Windows".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "functionapp config set -g {} -n {} --prewarmed-instance-count 4".format(resource_group, functionapp_name)
        ).assert_with_checks([JMESPathCheck("preWarmedInstanceCount", 4)])
...
class FunctionAppWithPlanE2ETest(ScenarioTest):
    ...
    def test_functionapp_e2e(self, resource_group, resource_group2):
        functionapp_name, functionapp_name2 = self.create_random_name("func-e2e", 24), self.create_random_name(
            "func-e2e", 24
        )
        ...
        self.cmd(
            "storage account create --name {} -g {} -l {} --sku Standard_LRS".format(
                storage, resource_group, WINDOWS_ASP_LOCATION_FUNCTIONAPP
            )
        )
        storage_account_id2 = self.cmd(
            "storage account create --name {} -g {} -l {} --sku Standard_LRS".format(
                storage2, resource_group2, WINDOWS_ASP_LOCATION_FUNCTIONAPP
            )
        ).get_output_in_json()["id"]

        self.cmd(
            "functionapp create -g {} -n {} -p {} -s {}".format(resource_group, functionapp_name, plan, storage),
            checks=[
                JMESPathCheck("state", "Running"),
                JMESPathCheck("name", functionapp_name),
                JMESPathCheck("hostNames[0]", functionapp_name + ".azurewebsites.net"),
        ...
        self.cmd(
            "functionapp create -g {} -n {} -p {} -s {}".format(
                resource_group2, functionapp_name2, plan_id, storage_account_id2
            )
        )
        self.cmd("functionapp delete -g {} -n {}".format(resource_group, functionapp_name))
    ...
    def test_functionapp_on_linux_app_service_java(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --runtime java --functions-version 3".format(
                resource_group, functionapp, plan, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        result = self.cmd(
            "functionapp list -g {}".format(resource_group),
            checks=[JMESPathCheck("length([])", 1), JMESPathCheck("[0].name", functionapp)],
        ).get_output_in_json()
        self.assertTrue("functionapp,linux" in result[0]["kind"])

        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("linuxFxVersion", "Java|8")],
        )
    ...
    def test_functionapp_on_linux_app_service_java_with_runtime_version(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --runtime java --runtime-version 11 --functions-version 3".format(
                resource_group, functionapp, plan, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("linuxFxVersion", "Java|11")],
        )
    ...
    def test_functionapp_on_linux_app_service_powershell(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --runtime powershell --functions-version 4".format(
                resource_group, functionapp, plan, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("linuxFxVersion", "PowerShell|7")],
        )
    ...
    def test_functionapp_on_linux_app_service_powershell_with_runtime_version(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --runtime powershell --runtime-version 7.0 --functions-version 4".format(
                resource_group, functionapp, plan, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
    ...
    def test_functionapp_on_linux_app_service_dotnet_isolated(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --runtime dotnet-isolated --functions-version 3".format(
                resource_group, functionapp, plan, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("[?name=='FUNCTIONS_WORKER_RUNTIME'].value|[0]", "dotnet-isolated")],
        )

        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("linuxFxVersion", "dotnet-isolated|5.0", case_sensitive=False)],
        )
...
class FunctionUpdatePlan(ScenarioTest):
    ...
    def test_move_plan_to_elastic(self, resource_group, storage_account):
        ...
        plan_result = self.cmd(
            "functionapp plan create -g {} -n {} --sku EP1".format(resource_group, ep_plan_name),
            checks=[JMESPathCheck("sku.name", "EP1")],
        ).get_output_in_json()

        self.cmd(
            "functionapp plan create -g {} -n {} --sku EP1".format(resource_group, second_plan_name),
            checks=[JMESPathCheck("sku.name", "EP1")],
        ).get_output_in_json()

        self.cmd(
            "functionapp plan create -g {} -n {} --sku S1".format(resource_group, s1_plan_name),
            checks=[JMESPathCheck("sku.name", "S1")],
        )

        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {}".format(
                resource_group, functionapp_name, second_plan_name, storage_account
            )
        ).assert_with_checks(
            [
                JMESPathCheck("state", "Running"),
        ...
        self.cmd(
            "functionapp update -g {} -n {} --plan {}".format(resource_group, functionapp_name, ep_plan_name)
        ).assert_with_checks(
            [
                JMESPathCheck("state", "Running"),
                JMESPathCheck("name", functionapp_name),
        ...
        self.cmd(
            "functionapp update -g {} -n {} --plan {}".format(resource_group, functionapp_name, s1_plan_name),
            expect_failure=True,
        )
...
class FunctionAppWithConsumptionPlanE2ETest(ScenarioTest):
    ...
    def test_functionapp_consumption_e2e(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {}".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "functionapp list -g {}".format(resource_group),
            checks=[JMESPathCheck("[0].kind", "functionapp"), JMESPathCheck("[0].name", functionapp_name)],
        )
        self.cmd(
            "functionapp show -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("kind", "functionapp"), JMESPathCheck("name", functionapp_name)],
        )
        self.cmd(
            "functionapp update -g {} -n {} --set clientCertMode=Optional".format(resource_group, functionapp_name),
...
class FunctionAppWithLinuxConsumptionPlanTest(ScenarioTest):
    ...
    def test_functionapp_consumption_linux(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Linux --functions-version 3 --runtime node".format(
                resource_group, functionapp_name, LINUX_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
                JMESPathCheck("state", "Running"),
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("[?name=='FUNCTIONS_WORKER_RUNTIME'].value|[0]", "node")],
        )
    ...
    def test_functionapp_consumption_linux_java(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Linux --runtime java --functions-version 3".format(
                resource_group, functionapp_name, LINUX_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("[?name=='FUNCTIONS_WORKER_RUNTIME'].value|[0]", "java")],
        )
    ...
    def test_functionapp_consumption_linux_powershell(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Linux --runtime powershell --functions-version 4".format(
                resource_group, functionapp_name, LINUX_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            ...
            checks=[
                ...
                JMESPathPatternCheck(
                    "[?name=='WEBSITE_CONTENTAZUREFILECONNECTIONSTRING'].value|[0]", ".+" + storage_account + "{1}.+"
                ),
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("linuxFxVersion", "PowerShell|7")],
        )
    ...
    def test_functionapp_consumption_linux_dotnet_isolated(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Linux --runtime dotnet-isolated --functions-version 3".format(
                resource_group, functionapp_name, LINUX_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("linuxFxVersion", "")],
        )
...
class FunctionAppOnWindowsWithRuntime(ScenarioTest):
    ...
    def test_functionapp_windows_runtime(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Windows --functions-version 3 --runtime node".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
    ...
    def test_functionapp_windows_runtime_java(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Windows --runtime java".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("javaVersion", "1.8")],
        )
    ...
    def test_functionapp_windows_runtime_powershell(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Windows --runtime powershell".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("[?name=='FUNCTIONS_WORKER_RUNTIME'].value|[0]", "powershell")],
        )

        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("powerShellVersion", "~7")],
        )
    ...
    def test_functionapp_windows_runtime_version(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Windows --functions-version 3 --runtime node --runtime-version 14".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
    ...
    def test_functionapp_windows_runtime_dotnet_isolated(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Windows --runtime dotnet-isolated --functions-version 3".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("netFrameworkVersion", "v4.0")],
        )
    ...
    def test_functionapp_windows_runtime_version_invalid(self, resource_group, storage_account):
        ...
        self.cmd(
            ...
            "--os-type Windows --runtime node --runtime-version 8.2".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            ),
            expect_failure=True,
        )
    ...
    def test_functionapp_windows_runtime_functions_version(self, resource_group, storage_account):
        functionapp_name = self.create_random_name("functionappwindowsruntime", 40)
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --functions-version 3 --os-type Windows --runtime node".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
    ...
    def test_functionapp_windows_runtime_custom_handler(self, resource_group, storage_account):
        functionapp_name = self.create_random_name("functionappwindowsruntime", 40)
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --functions-version 3 --os-type Windows --runtime custom".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
...
class FunctionAppWithAppInsightsKey(ScenarioTest):
    ...
    def test_functionapp_with_app_insights_key(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Windows"
            " --app-insights-key {}".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account, app_insights_key
            )
        ).assert_with_checks(
            [
                JMESPathCheck("state", "Running"),
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp_name)
        ).assert_with_checks([JMESPathCheck("[?name=='APPINSIGHTS_INSTRUMENTATIONKEY'].value|[0]", app_insights_key)])
...
class FunctionAppASPZoneRedundant(ScenarioTest):
    ...
    def test_functionapp_zone_redundant_plan(self, resource_group):
        ...
        self.cmd(
            "functionapp plan create -g {} -n {} -l {} -z --sku P1V2".format(
        ...
        self.cmd(
            "functionapp plan create -g {} -n {} -l {} -z --sku FREE".format(
        ...
        self.cmd(
            "functionapp plan create -g {} -n {} -l {} -z --sku P1V3".format(
...
class FunctionAppWithAppInsightsDefault(ScenarioTest):
    ...
    def test_functionapp_with_default_app_insights(self, resource_group, storage_account):
        ...
        self.assertTrue("AzureWebJobsDashboard" not in [kp["name"] for kp in app_set])
    ...
    def test_functionapp_with_no_default_app_insights(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type Windows --disable-app-insights".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.assertTrue("AzureWebJobsDashboard" in [kp["name"] for kp in app_set])
...
class FunctionAppOnLinux(ScenarioTest):
    ...
    def test_functionapp_on_linux(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --functions-version 3 --runtime node".format(
                resource_group, functionapp, plan, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("linuxFxVersion", "Node|14")],
        )
    ...
    def test_functionapp_on_linux_version(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --functions-version 3 --runtime node --runtime-version 14".format(
                resource_group, functionapp, plan, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
    ...
    def test_functionapp_on_linux_version_consumption(self, resource_group, storage_account):
        functionapp = self.create_random_name(prefix="functionapp-linux", length=24)
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type linux --runtime python --runtime-version 3.7".format(
                resource_group, functionapp, LINUX_ASP_LOCATION_FUNCTIONAPP, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("linuxFxVersion", "Python|3.7")],
        )
    ...
    def test_functionapp_on_linux_version_error(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --runtime python --runtime-version 4.8".format(
                resource_group, functionapp, plan, storage_account
            ),
            expect_failure=True,
        )
    ...
    def test_functionapp_on_linux_consumption_python_39(self, resource_group, storage_account):
        functionapp = self.create_random_name(prefix="functionapp-linux", length=24)
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --os-type linux --runtime python --functions-version 3 --runtime-version 3.9".format(
                resource_group, functionapp, LINUX_ASP_LOCATION_FUNCTIONAPP, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("linuxFxVersion", "Python|3.9")],
        )
    ...
    def test_functionapp_on_linux_functions_version(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp)
        ).assert_with_checks([JMESPathCheck("[?name=='FUNCTIONS_EXTENSION_VERSION'].value|[0]", "~3")])
    ...
    def test_functionapp_on_linux_custom_handler(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --functions-version 3 --runtime custom".format(
                resource_group, functionapp, plan, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp)
        ).assert_with_checks([JMESPathCheck("[?name=='FUNCTIONS_WORKER_RUNTIME'].value|[0]", "custom")])
    ...
    def test_functionapp_on_linux_functions_version_consumption(self, resource_group, storage_account):
        functionapp = self.create_random_name(prefix="functionapp-linux", length=24)
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --functions-version 3 --runtime node --os-type linux".format(
                resource_group, functionapp, LINUX_ASP_LOCATION_FUNCTIONAPP, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
    ...
    def test_functionapp_on_linux_dotnet_consumption(self, resource_group, storage_account):
        functionapp = self.create_random_name(prefix="functionapp-linux", length=24)
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --functions-version 3 --runtime dotnet --os-type linux".format(
                resource_group, functionapp, LINUX_ASP_LOCATION_FUNCTIONAPP, storage_account
            ),
            checks=[JMESPathCheck("name", functionapp)],
        )
        ...
        self.cmd(
            "functionapp config show -g {} -n {}".format(resource_group, functionapp),
            checks=[JMESPathCheck("linuxFxVersion", "dotnet|3.1")],
        )
        self.cmd(
            "functionapp config appsettings list -g {} -n {}".format(resource_group, functionapp)
        ).assert_with_checks([JMESPathCheck("[?name=='FUNCTIONS_EXTENSION_VERSION'].value|[0]", "~3")])
...
class FunctionAppServicePlan(ScenarioTest):
    ...
    def test_functionapp_app_service_plan(self, resource_group):
        ...
        self.cmd(
            "functionapp plan create -g {} -n {} --sku S1".format(resource_group, plan),
            checks=[JMESPathCheck("sku.name", "S1")],
        )
    ...
    def test_functionapp_elastic_plan(self, resource_group):
        plan = self.create_random_name(prefix="funcappplan", length=24)
        self.cmd(
            "functionapp plan create -g {} -n {} --sku EP1 --min-instances 4 --max-burst 12".format(
                resource_group, plan
            ),
            checks=[
        ...
        self.cmd(
            "functionapp plan update -g {} -n {} --min-instances 5 --max-burst 11".format(resource_group, plan),
            checks=[
        ...
        self.cmd(
            "functionapp plan show -g {} -n {} ".format(resource_group, plan),
            checks=[
                JMESPathCheck("maximumElasticWorkerCount", 11),
                JMESPathCheck("sku.name", "EP1"),
                JMESPathCheck("sku.capacity", 1),
...
class FunctionAppServicePlanLinux(ScenarioTest):
    ...
    def test_functionapp_app_service_plan_linux(self, resource_group):
        ...
        self.cmd(
            "functionapp plan create -g {} -n {} --sku S1 --is-linux".format(resource_group, plan),
            checks=[JMESPathCheck("sku.name", "S1"), JMESPathCheck("kind", "linux")],
        )
...
class FunctionAppSlotTests(ScenarioTest):
    ...
    def test_functionapp_slot_appsetting_update(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp deployment slot create -g {} -n {} --slot {}".format(resource_group, functionapp, slotname),
            checks=[JMESPathCheck("name", slotname)],
        )
        self.cmd(
            "functionapp config appsettings set -g {} -n {} --slot {} --slot-settings FOO=BAR".format(
        ...
        self.cmd(
            "functionapp config appsettings list -g {} -n {} --slot {}".format(resource_group, functionapp, slotname),
            checks=[
                JMESPathCheck("[?name=='FOO'].value|[0]", "BAR"),
                JMESPathCheck("[?name=='FOO'].slotSetting|[0]", True),
            ],
    ...
    def test_functionapp_slot_swap(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp config appsettings set -g {} -n {} --slot {} --settings FOO=BAR".format(
                resource_group, functionapp, slotname
            ),
            checks=[JMESPathCheck("[?name=='FOO'].value|[0]", "BAR")],
        )
        self.cmd(
            "functionapp deployment slot swap -g {} -n {} --slot {} --action swap".format(
                resource_group, functionapp, slotname
            )
        )
        self.cmd(
...
class FunctionAppKeysTests(ScenarioTest):
    ...
    def test_functionapp_keys_set(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} -c {} -s {} --functions-version 3".format(
                resource_group, functionapp_name, WINDOWS_ASP_LOCATION_FUNCTIONAPP, storage_account
            )
        ).assert_with_checks(
            [
        ...
        self.cmd(
            "functionapp keys set -g {} -n {} --key-name {} --key-value {} --key-type {}".format(
                resource_group, functionapp_name, key_name, key_value, key_type
            )
        ).assert_with_checks(
            [JMESPathCheck("name", key_name), JMESPathCheck("type", "Microsoft.Web/sites/host/functionKeys")]
        )
    ...
    def test_functionapp_keys_list(self, resource_group, storage_account):
        ...
        self.cmd("functionapp keys list -g {} -n {}".format(resource_group, functionapp_name)).assert_with_checks(
    ...
    def test_functionapp_keys_delete(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp keys delete -g {} -n {} --key-name {} --key-type {}".format(
                resource_group, functionapp_name, key_name, key_type
            )
        )
    ...
    def test_functionapp_keys_set_slot(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp deployment slot create -g {} -n {} --slot {}".format(
                resource_group, functionapp_name, slot_name
            )
        ).assert_with_checks([JMESPathCheck("name", slot_name), JMESPathCheck("type", "Microsoft.Web/sites/slots")])

        self.cmd(
            "functionapp keys set -g {} -n {} -s {} --key-name {} --key-value {} --key-type {}".format(
                resource_group, functionapp_name, slot_name, key_name, key_value, key_type
            )
        ).assert_with_checks(
            [JMESPathCheck("name", key_name), JMESPathCheck("type", "Microsoft.Web/sites/host/functionKeys")]
        )
...
class FunctionAppFunctionKeysTests(LiveScenarioTest):
    ...
    def test_functionapp_function_keys_set(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {} --runtime dotnet".format(
                resource_group, functionapp_name, plan_name, storage_account
            )
        )
        ...
        self.cmd(
            'functionapp deployment source config-zip -g {} -n {} --src "{}"'.format(
                resource_group, functionapp_name, zip_file
            )
        ).assert_with_checks(
            [JMESPathCheck("status", 4), JMESPathCheck("deployer", "ZipDeploy"), JMESPathCheck("complete", True)]
        )
        ...
        self.cmd(
            "functionapp function keys set -g {} -n {} --function-name {} --key-name {} --key-value {}".format(
                resource_group, functionapp_name, function_name, key_name, key_value
            )
        ).assert_with_checks([JMESPathCheck("name", key_name)])
    ...
    def test_functionapp_function_keys_list(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp function keys list -g {} -n {} --function-name {}".format(
                resource_group, functionapp_name, function_name
            )
        ).assert_with_checks([JMESPathCheck("{}".format(key_name), key_value)])
    ...
    def test_functionapp_function_keys_delete(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp function show -g {} -n {} --function-name {}".format(
                resource_group, functionapp_name, function_name
            )
        )
        ...
        self.cmd(
            "functionapp function keys delete -g {} -n {} --function-name {} --key-name {}".format(
                resource_group, functionapp_name, function_name, key_name
            )
        )
        ...
        ).assert_with_checks([JMESPathCheck("{}".format(key_name), None)])
...
class FunctionAppFunctionTests(LiveScenarioTest):
    ...
    def test_functionapp_function_show(self, resource_group, storage_account):
        ...
        self.cmd(
            ...
            [
                ...
                JMESPathCheck(
                    "scriptHref",
                    "https://{}.azurewebsites.net/admin/vfs/site/wwwroot/{}/run.csx".format(
    ...
    def test_functionapp_windows_zip_deploy(self, resource_group, storage_account):
        ...
        self.cmd(
            'functionapp deployment source config-zip --build-remote -g {} -n {} --src "{}"'.format(
                resource_group, functionapp_name, zip_file
            )
        ).assert_with_checks(
            [JMESPathCheck("status", 4), JMESPathCheck("deployer", "ZipDeploy"), JMESPathCheck("complete", True)]
    ...
    def test_functionapp_function_delete(self, resource_group, storage_account):
        ...
        ).assert_with_checks([JMESPathCheck("config.bindings[0].type", "httpTrigger")])

        self.cmd(
            "functionapp function delete -g {} -n {} --function-name {}".format(
                resource_group, functionapp_name, function_name
            )
        )
...
class FunctionappDeploymentLogsScenarioTest(LiveScenarioTest):
    ...
    def test_functionapp_show_deployment_logs(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp log deployment show -g {} -n {}".format(resource_group, functionapp_name),
        ...
        self.cmd(
            "functionapp log deployment show -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("length(@) > `0`", True)],
        )

        self.cmd(
            "functionapp log deployment show -g {} -n {} --deployment-id={}".format(
                resource_group, functionapp_name, deployment_1["id"]
            ),
            checks=[JMESPathCheck("length(@) > `0`", True)],
        )
    ...
    def test_functionapp_list_deployment_logs(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp log deployment list -g {} -n {}".format(resource_group, functionapp_name),
            checks=[JMESPathCheck("length(@)", 0)],
        )
...
class FunctionappIdentityTest(ScenarioTest):
    ...
    def test_functionapp_assign_system_identity(self, resource_group, storage_account):
        ...
        with mock.patch("azure.cli.core.commands.arm._gen_guid", side_effect=self.create_guid):
            result = self.cmd(
                "functionapp identity assign -g {} -n {} --role {} --scope {}".format(
                    resource_group, functionapp_name, role, scope
                )
            ).get_output_in_json()
            self.cmd(
                "functionapp identity show -g {} -n {}".format(resource_group, functionapp_name),
        ...
        self.cmd(
            "functionapp identity show -g {} -n {}".format(resource_group, functionapp_name), checks=self.is_empty()
        )
    ...
    def test_functionapp_assign_user_identity(self, resource_group, storage_account):
        ...
        result = self.cmd(
            "functionapp identity assign -g {} -n {} --identities {}".format(
        ...
        self.cmd(
            "functionapp identity remove -g {} -n {} --identities {}".format(
                resource_group, functionapp_name, msi_result["id"]
            )
        )
        self.cmd(
    ...
    def test_functionapp_remove_identity(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp identity assign -g {} -n {} --identities [system] {} {}".format(
        ...
        result = self.cmd(
            "functionapp identity remove -g {} -n {} --identities {}".format(
                resource_group, functionapp_name, msi2_result["id"]
            )
        ).get_output_in_json()
        self.cmd(
            "functionapp identity show -g {} -n {}".format(resource_group, functionapp_name),
        ...
        self.cmd(
            "functionapp identity remove -g {} -n {} --identities [system] {}".format(
                resource_group, functionapp_name, msi_result["id"]
            )
        )
        self.cmd(
...
class FunctionappNetworkConnectionTests(ScenarioTest):
    ...
    def test_functionapp_vnetE2E(self, resource_group, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} -s {}".format(
                resource_group, functionapp_name, plan, storage_account
            )
        )
        self.cmd(
            "functionapp vnet-integration add -g {} -n {} --vnet {} --subnet {}".format(
                resource_group, functionapp_name, vnet_name, subnet_name
            )
        )
        self.cmd(
            "functionapp vnet-integration list -g {} -n {}".format(resource_group, functionapp_name),
        ...
        self.cmd(
            "functionapp vnet-integration add -g {} -n {} --vnet {} --subnet {} --slot {}".format(
                resource_group, functionapp_name, vnet_name, subnet_name, slot_functionapp_name
            )
        )
        self.cmd(
            "functionapp vnet-integration list -g {} -n {} --slot {}".format(
                resource_group, functionapp_name, slot_functionapp_name
            ),
            checks=[JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", subnet_name)],
        )
        ...
        self.cmd(
            "functionapp vnet-integration remove -g {} -n {} --slot {}".format(
                resource_group, functionapp_name, slot_functionapp_name
            )
        )
        self.cmd(
    ...
    def test_functionapp_vnet_duplicate_name(self, storage_account, resource_group, rg2, rg3, rg4, rg5):
        ...
        self.cmd(
            ...
            checks=[JMESPathCheck("subnetResourceId", subnet_id)],
        )
        self.cmd(
            "functionapp vnet-integration list -g {} -n {}".format(resource_group, functionapp_name),
            checks=[
    ...
    def test_functionapp_create_vnetE2E(self, resource_group, storage_account):
        ...
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(resource_group, plan))
        self.cmd(
            "functionapp create -g {} -n {} --plan {} --vnet {} --subnet {} -s {}".format(
                resource_group, functionapp_name, plan, vnet_name, subnet_name, storage_account
            )
        )
        self.cmd(
            "functionapp vnet-integration list -g {} -n {}".format(resource_group, functionapp_name),
    ...
    def test_functionapp_create_with_vnet_by_subnet_rid(self, functionapp_rg, vnet_rg, storage_account):
        ...
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(functionapp_rg, plan))
        self.cmd(
            "functionapp create -g {} -n {} --plan {} --subnet {} -s {}".format(
                functionapp_rg, functionapp_name, plan, subnet_id, storage_account
            )
        )
        self.cmd(
            "functionapp vnet-integration list -g {} -n {}".format(functionapp_rg, functionapp_name),
            checks=[JMESPathCheck("length(@)", 1), JMESPathCheck("[0].name", subnet_name)],
        )
        self.cmd("functionapp vnet-integration remove -g {} -n {}".format(functionapp_rg, functionapp_name))
        self.cmd(
            "functionapp vnet-integration list -g {} -n {}".format(functionapp_rg, functionapp_name),
            checks=[JMESPathCheck("length(@)", 0)],
        )
    ...
    def test_functionapp_create_with_vnet_by_vnet_rid(self, functionapp_rg, vnet_rg, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} --vnet {} --subnet {} -s {}".format(
                functionapp_rg, functionapp_name, plan, vnet_id, subnet_name, storage_account
            )
        )
        self.cmd(
            "functionapp vnet-integration list -g {} -n {}".format(functionapp_rg, functionapp_name),
    ...
    def test_functionapp_create_with_vnet_wrong_rg(self, functionapp_rg, vnet_rg, storage_account):
        ...
        self.cmd(
            "functionapp create -g {} -n {} --plan {} --vnet {} --subnet {} -s {}".format(
                functionapp_rg, functionapp_name, plan, vnet_name, subnet_name, storage_account
            ),
            expect_failure=True,
        )
    ...
    def test_functionapp_create_with_vnet_no_subnet(self, resource_group, storage_account):
        ...
        self.cmd("appservice plan create -g {} -n {} --sku P1V2".format(resource_group, plan))
        self.cmd(
            "functionapp create -g {} -n {} --plan {} --vnet {} -s {}".format(
                resource_group, functionapp_name, plan, vnet_name, subnet_name, storage_account
            ),
            expect_failure=True,
        )
