...
def create_appserviceenvironment_arm(
    cmd,
    resource_group_name,
    name,
    subnet,
    ...
    if kind == "ASEv2":
        ...
        if not ignore_network_security_group:
            _ensure_network_security_group(
...
def delete_appserviceenvironment(cmd, name, resource_group_name=None, no_wait=False):
    ...
    return sdk_no_wait(no_wait, ase_client.begin_delete, resource_group_name=resource_group_name, name=name)


def update_appserviceenvironment(
    cmd,
    name,
    resource_group_name=None,
    front_end_scale_factor=None,
    front_end_sku=None,
    allow_new_private_endpoint_connections=None,
    no_wait=False,
):
    ase_client = _get_ase_client_factory(cmd.cli_ctx)
    if resource_group_name is None:
    ...
    if ase_def.kind.lower() == "asev3":
        if allow_new_private_endpoint_connections is not None:
            ...
            if ase_networking_conf.allow_new_private_endpoint_connections != allow_new_private_endpoint_connections:
                ...
                return ase_client.update_ase_networking_configuration(
                    resource_group_name=resource_group_name, name=name, ase_networking_configuration=ase_networking_conf
                )
        ...
        if front_end_scale_factor is not None or front_end_sku is not None:
            ...
            return sdk_no_wait(
                no_wait,
                ase_client.begin_create_or_update,
                resource_group_name=resource_group_name,
                name=name,
                hosting_environment_envelope=ase_def,
            )
...
def _map_worker_sku(sku_name):
    switcher = {"I1": "Standard_D1_V2", "I2": "Standard_D2_V2", "I3": "Standard_D3_V2"}
    ...
    if subnet_obj.route_table is None or force:
        ...
        if not rt_found:
            ...
            poller = network_client.routes.begin_create_or_update(
                resource_group_name, ase_route_table_name, ase_route_name, internet_route
            )
            LongRunningOperation(cli_ctx)(poller)
...
def _create_asev2_nsg_rules(cli_ctx, resource_group_name, ase_nsg_name, subnet_address_prefix):
    ...
    _create_nsg_rule(
        ...
        resource_group_name,
        ase_nsg_name,
        "Inbound-load-balancer-keep-alive",
        105,
