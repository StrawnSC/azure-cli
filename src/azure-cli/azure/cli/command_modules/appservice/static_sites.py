...
def disconnect_staticsite(cmd, name, resource_group_name=None, no_wait=False):
    ...
    return sdk_no_wait(no_wait, client.begin_detach_static_site, resource_group_name=resource_group_name, name=name)


def reconnect_staticsite(
    cmd, name, source, branch, token=None, resource_group_name=None, login_with_github=False, no_wait=False
):
    client = _get_staticsites_client_factory(cmd.cli_ctx)
    if not resource_group_name:
        resource_group_name = _get_resource_group_name_of_staticsite(client, name)
...
def delete_staticsite_environment(cmd, name, environment_name="default", resource_group_name=None, no_wait=False):
    ...
    return sdk_no_wait(no_wait, client.begin_delete_static_site_build, resource_group_name, name, environment_name)
...
def set_staticsite_domain(
    cmd, name, hostname, resource_group_name=None, no_wait=False, validation_method="cname-delegation"
):
    ...
    client.begin_validate_custom_domain_can_be_added_to_static_site(
        resource_group_name, name, hostname, domain_envelope
    )

    if validation_method.lower() == "dns-txt-token":
        validation_cmd = "az staticwebapp hostname get -n {} -g {} " '--hostname {} --query "validationToken"'.format(
            name, resource_group_name, hostname
        )
        logger.warning(
            "To get the TXT validation token, please run '%s'. "
            "It may take a few minutes to generate the validation token.",
            validation_cmd,
        )
...
def get_staticsite_domain(cmd, name, hostname, resource_group_name):
    client = _get_staticsites_client_factory(cmd.cli_ctx)
    return client.get_static_site_custom_domain(
...
def assign_identity(cmd, resource_group_name, name, assign_identities=None, role="Contributor", scope=None):
    ...
    def setter(staticsite):
        if staticsite.identity and staticsite.identity.type == ResourceIdentityType.system_assigned_user_assigned:
            ...
            staticsite.identity
            and staticsite.identity.type == ResourceIdentityType.system_assigned
            and external_identities
        ):  # pylint: disable=line-too-long
            identity_types = ResourceIdentityType.system_assigned_user_assigned
        elif (
            staticsite.identity
            and staticsite.identity.type == ResourceIdentityType.user_assigned
...
def remove_identity(cmd, resource_group_name, name, remove_identities=None):
    ...
    def setter(staticsite):
        ...
        if remove_local_identity:
            staticsite.identity.type = (
                IdentityType.none
                if staticsite.identity.type == IdentityType.system_assigned
                or staticsite.identity.type == IdentityType.none
...
def set_staticsite_app_settings(cmd, name, setting_pairs, resource_group_name=None):
    ...
    return client.create_or_update_static_site_app_settings(resource_group_name, name, app_settings=settings)
...
def delete_staticsite_app_settings(cmd, name, setting_names, resource_group_name=None):
    ...
    return client.create_or_update_static_site_app_settings(resource_group_name, name, app_settings=app_settings)
...
def update_staticsite_users(
    cmd, name, roles, authentication_provider=None, user_details=None, user_id=None, resource_group_name=None
):
    ...
    return client.update_static_site_user(
        resource_group_name, name, authentication_provider, user_id, static_site_user_envelope=user_envelope
    )


def create_staticsites(
    cmd,
    resource_group_name,
    name,
    location,
    ...
    build = StaticSiteBuildProperties(
        app_location=app_location, api_location=api_location, app_artifact_location=output_location
    )
    ...
    return sdk_no_wait(
        no_wait,
        client.begin_create_or_update_static_site,
        resource_group_name=resource_group_name,
        name=name,
...
def update_staticsite(cmd, name, source=None, branch=None, token=None, tags=None, sku=None, no_wait=False):
...
def delete_staticsite(cmd, name, resource_group_name=None, no_wait=False):
    ...
    return sdk_no_wait(no_wait, client.begin_delete_static_site, resource_group_name=resource_group_name, name=name)
...
def _get_staticsite_location(client, static_site_name, resource_group_name):
    ...
    raise CLIError(
        "Static site was '{}' not found in subscription and resource group '{}'.".format(
            static_site_name, resource_group_name
        )
    )
...
def _find_user_id_and_authentication_provider(
    client, resource_group_name, name, user_id, authentication_provider, user_details
):
    users = client.list_static_site_users(resource_group_name, name, "all")
    for user in users:
...
def reset_staticsite_api_key(cmd, name, resource_group_name=None):
    ...
    return client.reset_static_site_api_key(
        resource_group_name=resource_group_name, name=name, reset_properties_envelope=reset_envelope
    )
...
def link_user_function(cmd, name, resource_group_name, function_resource_id, force=False):
    ...
    function = StaticSiteUserProvidedFunctionAppARMResource(
        function_app_resource_id=function_resource_id, function_app_region=function_location
    )
...
def unlink_user_function(cmd, name, resource_group_name):
    ...
    return client.detach_user_provided_function_app_from_static_site(
        name=name, resource_group_name=resource_group_name, function_app_name=function_name
    )
