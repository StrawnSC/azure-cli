...
from ._validators import (
    ...
    validate_asp_create,
    validate_add_vnet,
    validate_front_end_scale_factor,
    validate_ase_create,
    validate_ip_address,
    validate_service_tag,
    validate_public_cloud,
)
...
def load_arguments(self, _):
    ...
    functionapp_name_arg_type = CLIArgumentType(
        options_list=["--name", "-n"],
        metavar="NAME",
    ...
    logicapp_name_arg_type = CLIArgumentType(
        options_list=["--name", "-n"],
        metavar="NAME",
        help="name of the logic app.",
        local_context_attribute=LocalContextAttribute(name="logicapp_name", actions=[LocalContextAction.GET]),
    )
    name_arg_type_dict = {"functionapp": functionapp_name_arg_type, "logicapp": logicapp_name_arg_type}
    ...
    static_web_app_sku_arg_type = CLIArgumentType(
        help="The pricing tiers for Static Web App", arg_type=get_enum_type(["Free", "Standard"])
    )
    ...
    with self.argument_context("appservice plan") as c:
        ...
        c.argument(
            "admin_site_name", help="The name of the admin web app.", deprecate_info=c.deprecate(expiration="0.2.17")
        )
        c.ignore("max_burst")
    ...
    with self.argument_context("appservice plan create") as c:
        c.argument(
            ...
            local_context_attribute=LocalContextAttribute(
                name="plan_name", actions=[LocalContextAction.SET], scopes=["appservice", "webapp", "functionapp"]
            ),
        )
        c.argument("number_of_workers", help="Number of workers to be allocated.", type=int, default=1)
        c.argument(
            "app_service_environment",
            options_list=["--app-service-environment", "-e"],
        ...
        c.argument(
            ...
            action="store_true",
            required=False,
            help="Enable per-app scaling at the "
        ...
        c.argument(
            ...
            help="Enable zone redundancy for high availability. Cannot be changed after plan creation. Minimum instance count is 3.",
        )
    ...
    with self.argument_context("appservice plan update") as c:
        ...
        c.argument(
            ...
            is_preview=True,
            help='Enable or disable automatic scaling. Set to "true" to enable elastic scale for this plan, or "false" to disable elastic scale for this plan. The SKU must be a Premium V2 SKU (P1V2, P2V2, P3V2) or a Premium V3 SKU (P1V3, P2V3, P3V3)',
        )
        c.argument(
            "max_elastic_worker_count",
            options_list=["--max-elastic-worker-count", "-m"],
            type=int,
            is_preview=True,
            help="Maximum number of instances that the plan can scale out to. The plan must be an elastic scale plan.",
        )
    ...
    with self.argument_context("webapp create") as c:
        c.argument(
            "name",
            options_list=["--name", "-n"],
            help="name of the new web app",
            validator=validate_site_create,
            local_context_attribute=LocalContextAttribute(
                name="web_name", actions=[LocalContextAction.SET], scopes=["webapp", "cupertino"]
            ),
        )
        c.argument("startup_file", help="Linux only. The web's startup file")
        c.argument(
            "docker_registry_server_user",
            options_list=["--docker-registry-server-user", "-s"],
            help="the container registry server username",
        )
        c.argument(
            "docker_registry_server_password",
            options_list=["--docker-registry-server-password", "-w"],
        ...
        c.argument(
            ...
            help="Linux only. Config file for multicontainer apps. (local or remote)",
        )
        c.argument(
            "runtime",
            options_list=["--runtime", "-r"],
            help='canonicalized web runtime in the format of Framework:Version, e.g. "PHP:7.2". Allowed delimiters: "|" or ":". If using powershell, please use the ":" delimiter or be sure to properly escape the "|" character. '
        ...
        c.argument(
            "subnet",
            help="Name or resource ID of the pre-existing subnet to have the webapp join. The --vnet is argument also needed if specifying subnet by name.",
        )
        c.ignore("language")
    ...
    with self.argument_context("webapp list-instances") as c:
        ...
        c.argument(
            "slot",
            options_list=["--slot", "-s"],
            help="Name of the web app slot. Default to the productions slot if not specified.",
        )
    ...
    with self.argument_context("webapp list-runtimes") as c:
        c.argument(
            "linux",
            action="store_true",
            help="list runtime stacks for linux based web apps",
            deprecate_info=c.deprecate(redirect="--os-type"),
        )
        c.argument(
            "os_type",
            options_list=["--os", "--os-type"],
    ...
    with self.argument_context("webapp deleted restore") as c:
        ...
        c.argument(
            "restore_content_only", action="store_true", help="restore only deleted files without web app settings"
        )
    ...
    with self.argument_context("webapp update") as c:
        ...
        c.argument(
            ...
            help="If true web app hostname is not registered with DNS on creation",
            arg_type=get_three_state_flag(return_label=True),
            deprecate_info=c.deprecate(expiration="3.0.0"),
        )
        c.argument(
            "minimum_elastic_instance_count",
            options_list=["--minimum-elastic-instance-count", "-i"],
            type=int,
            is_preview=True,
            help="Minimum number of instances. App must be in an elastic scale App Service Plan.",
        )
        c.argument(
            "prewarmed_instance_count",
            options_list=["--prewarmed-instance-count", "-w"],
            type=int,
            is_preview=True,
            help="Number of preWarmed instances. App must be in an elastic scale App Service Plan.",
        )
    ...
    for scope in ["webapp", "functionapp", "logicapp"]:
        with self.argument_context(scope + " create") as c:
            ...
            c.argument(
                "deployment_zip", options_list=["--deployment-zip", "-z"], help="perform deployment using zip file"
            )
            c.argument(
    ...
    for scope in ["webapp", "functionapp"]:
        ...
        with self.argument_context(scope + " deployment source") as c:
            c.argument(
                "manual_integration", action="store_true", help="disable automatic sync between source control and web"
            )
            c.argument(
                "repo_url",
            ...
            c.argument(
                "github_action", options_list=["--github-action"], help="If using github action, default to False"
            )
        ...
        with self.argument_context(scope + " config appsettings list") as c:
            c.argument(
                "name",
                arg_type=(webapp_name_arg_type if scope == "webapp" else functionapp_name_arg_type),
                id_part=None,
            )
        ...
        with self.argument_context(scope + " config set") as c:
            ...
            c.argument(
                "linux_fx_version",
                help='The runtime stack used for your linux-based webapp, e.g., "RUBY|2.5.5", "NODE|12LTS", "PHP|7.2", "DOTNETCORE|2.1". See https://aka.ms/linux-stacks for more info.',
            )
            c.argument(
                "windows_fx_version",
            ...
            c.argument(
                ...
                options_list=["--prewarmed-instance-count"],
            ...
            c.argument(
                "http20_enabled",
                help="configures a web site to allow clients to connect over http2.0.",
                arg_type=get_three_state_flag(return_label=True),
            )
            c.argument(
                "app_command_line",
            ...
            c.argument(
                "ftps_state",
                help="Set the Ftps state value for an app. Default value is 'AllAllowed'.",
                arg_type=get_enum_type(FTPS_STATE_TYPES),
            )
            c.argument(
                "vnet_route_all_enabled",
                help="Configure regional VNet integration to route all traffic to the VNet.",
                arg_type=get_three_state_flag(return_label=True),
            )
            c.argument(
    ...
    with self.argument_context("webapp deployment list-publishing-profiles") as c:
        c.argument(
            "xml", options_list=["--xml"], required=False, help="retrieves the publishing profile details in XML format"
        )
    with self.argument_context("webapp deployment slot") as c:
        ...
        c.argument(
            "preserve_vnet",
            help="preserve Virtual Network to the slot during swap, default to 'true'",
            arg_type=get_three_state_flag(return_label=True),
        )
    ...
    with self.argument_context("webapp deployment github-actions") as c:
        ...
        c.argument(
            "repo",
            help="The GitHub repository to which the workflow file will be added. In the format: <owner>/<repository-name>",
        )
        c.argument(
            "token",
            help="A Personal Access Token with write access to the specified repository. For more information: https://help.github.com/en/github/authenticating-to-github/creating-a-personal-access-token-for-the-command-line",
        )
        c.argument(
            "slot",
            options_list=["--slot", "-s"],
        ...
        c.argument(
            ...
            help='The branch to which the workflow file will be added. Defaults to "master" if not specified.',
        )
        c.argument(
            "login_with_github",
            help="Interactively log in with Github to retrieve the Personal Access Token",
            action="store_true",
        )
    ...
    with self.argument_context("webapp deployment github-actions add") as c:
        c.argument(
            "runtime",
            options_list=["--runtime", "-r"],
            help='Canonicalized web runtime in the format of Framework|Version, e.g. "PHP|5.6". Use "az webapp list-runtimes" for available list.',
        )
        c.argument(
            "force",
            options_list=["--force", "-f"],
            help="When true, the command will overwrite any workflow file with a conflicting name.",
            action="store_true",
        )
    ...
    with self.argument_context("webapp log config") as c:
        c.argument(
            "application_logging",
            help="configure application logging",
        ...
        c.argument(
            "level", help="logging level", arg_type=get_enum_type(["error", "warning", "information", "verbose"])
        )
        c.argument(
            "web_server_logging", help="configure Web server logging", arg_type=get_enum_type(["off", "filesystem"])
        )
        c.argument(
    ...
    with self.argument_context("webapp log deployment show") as c:
        ...
        c.argument(
            "deployment_id",
            options_list=["--deployment-id"],
            help="Deployment ID. If none specified, returns the deployment logs of the latest deployment.",
        )
    ...
    for scope in ["appsettings", "connection-string"]:
        with self.argument_context("webapp config " + scope) as c:
            ...
            c.argument(
                "slot_settings",
                nargs="+",
                help="space-separated slot {} in a format of either `<name>=<value>` or `@<json_file>`".format(scope),
            )
    ...
    with self.argument_context("webapp config storage-account") as c:
        c.argument(
            "custom_id", options_list=["--custom-id", "-i"], help="name of the share configured within the web app"
        )
        c.argument(
            "storage_type",
    ...
    with self.argument_context("webapp config backup") as c:
        ...
        c.argument(
            "webapp_name",
            help="The name of the web app",
            local_context_attribute=LocalContextAttribute(name="web_name", actions=[LocalContextAction.GET]),
        )
        c.argument("db_name", help="Name of the database in the backup", arg_group="Database")
        c.argument(
            "db_connection_string", help="Connection string for the database in the backup", arg_group="Database"
        )
        c.argument(
            "db_type", help="Type of database in the backup", arg_group="Database", arg_type=get_enum_type(DatabaseType)
        )
    ...
    with self.argument_context("webapp config backup create") as c:
        c.argument(
            ...
            local_context_attribute=LocalContextAttribute(
                name="backup_name", actions=[LocalContextAction.SET], scopes=["webapp"]
            ),
        )
    ...
    with self.argument_context("webapp config backup update") as c:
        ...
        c.argument(
            "frequency", help="How often to backup. Use a number followed by d or h, e.g. 5d = 5 days, 2h = 2 hours"
        )
        c.argument(
    ...
    with self.argument_context("webapp config backup restore") as c:
        ...
        c.argument(
            "overwrite", help="Overwrite the source web app, if --target-name is not specified", action="store_true"
        )
        c.argument(
            "ignore_hostname_conflict", help="Ignores custom hostnames stored in the backup", action="store_true"
        )
    ...
    with self.argument_context("webapp auth update") as c:
        ...
        c.argument(
            ...
            options_list=["--aad-client-secret-certificate-thumbprint", "--thumbprint"],
            arg_group="Azure Active Directory",
        ...
        c.argument(
            ...
            nargs="+",
            help="One or more facebook authentication scopes (space-delimited).",
            arg_group="Facebook",
        )
        c.argument(
        ...
        c.argument(
            "google_client_id", arg_group="Google", help="Application ID to integrate Google Sign-in into your web app"
        )
        ...
        c.argument(
            ...
            nargs="+",
            help="One or more Microsoft authentification scopes (space-delimited).",
            arg_group="Microsoft",
        )
    ...
    with self.argument_context("webapp up") as c:
        ...
        c.argument(
            "dryrun",
            help="show summary of the create and deploy operation instead of executing it",
            default=False,
            action="store_true",
        )
        c.argument("location", arg_type=get_location_type(self.cli_ctx))
        c.argument(
            "launch_browser",
            help="Launch the created app using the default browser",
            default=False,
            action="store_true",
            options_list=["--launch-browser", "-b"],
        )
        c.argument(
            "logs",
        ...
        c.argument(
            "app_service_environment",
            options_list=["--app-service-environment", "-e"],
            help="name of the (pre-existing) App Service Environment to deploy to. Requires an Isolated V2 sku [I1v2, I2v2, I3v2]",
        )
    ...
    with self.argument_context("webapp ssh") as c:
        c.argument(
            "port",
            options_list=["--port", "-p"],
            help="Port for the remote connection. Default: Random available port",
            type=int,
        )
        c.argument("timeout", options_list=["--timeout", "-t"], help="timeout in seconds. Defaults to none", type=int)
        c.argument(
            "instance", options_list=["--instance", "-i"], help="Webapp instance to connect to. Defaults to none."
        )
    ...
    with self.argument_context("webapp vnet-integration") as c:
        ...
        c.argument(
            "vnet",
            help="The name or resource ID of the Vnet",
            local_context_attribute=LocalContextAttribute(name="vnet_name", actions=[LocalContextAction.GET]),
        )
        c.argument(
            "subnet",
            help="The name or resource ID of the subnet",
            local_context_attribute=LocalContextAttribute(name="subnet_name", actions=[LocalContextAction.GET]),
        )
        c.argument(
            "skip_delegation_check",
            help="Skip check if you do not have permission or the VNet is in another subscription.",
            arg_type=get_three_state_flag(return_label=True),
        )
    ...
    with self.argument_context("webapp deploy") as c:
        ...
        c.argument(
            ...
            help='Path of the artifact to be deployed. Ex: "myapp.zip" or "/myworkspace/apps/myapp.war"',
        )
        c.argument(
            "src_url",
            options_list=["--src-url"],
            help='URL of the artifact. The webapp will pull the artifact from this URL. Ex: "http://mysite.com/files/myapp.war?key=123"',
        )
        c.argument(
            "target_path",
            options_list=["--target-path"],
            help='Absolute path that the artifact should be deployed to. Defaults to "home/site/wwwroot/" Ex: "/home/site/deployments/tools/", "/home/site/scripts/startup-script.sh".',
        )
        c.argument(
            "artifact_type",
            options_list=["--type"],
            help="Used to override the type of artifact being deployed.",
            choices=["war", "jar", "ear", "lib", "startup", "static", "zip"],
        )
        c.argument(
            "is_async",
            options_list=["--async"],
            help="If true, the artifact is deployed asynchronously. (The command will exit once the artifact is pushed to the web app.)",
            choices=["true", "false"],
        )
        c.argument(
            "restart",
            options_list=["--restart"],
            help="If true, the web app will be restarted following the deployment. Set this to false if you are deploying multiple artifacts and do not want to restart the site on the earlier deployments.",
            choices=["true", "false"],
        )
        c.argument(
            "clean",
            options_list=["--clean"],
            help="If true, cleans the target directory prior to deploying the file(s). Default value is determined based on artifact type.",
            choices=["true", "false"],
        )
        c.argument(
            "ignore_stack",
            options_list=["--ignore-stack"],
            help="If true, any stack-specific defaults are ignored.",
            choices=["true", "false"],
        )
    ...
    with self.argument_context("functionapp deploy") as c:
        ...
        c.argument(
            "restart",
            options_list=["--restart"],
            help="If true, the web app will be restarted following the deployment, default value is true. Set this to false if you are deploying multiple artifacts and do not want to restart the site on the earlier deployments.",
            choices=["true", "false"],
        )
        c.argument(
            "clean",
    ...
    for scope in ["functionapp", "logicapp"]:
        ...
        with self.argument_context(scope) as c:
            c.ignore("app_instance")
            c.argument(
                "name", arg_type=name_arg_type_dict[scope], id_part="name", help="name of the {} app".format(app_type)
            )
            c.argument(
                "slot",
                options_list=["--slot", "-s"],
        ...
        with self.argument_context(scope + " create") as c:
            c.argument(
                ...
                configured_default="appserviceplan",
                completer=get_resource_name_completion_list("Microsoft.Web/serverFarms"),
                help="name or resource id of the {} app service plan. Use 'appservice plan create' to get one. If using an App Service plan from a different resource group, the full resource id must be used and not the plan name.".format(
                    scope
                ),
                local_context_attribute=LocalContextAttribute(name="plan_name", actions=[LocalContextAction.GET]),
            )
            c.argument(
                "name",
                options_list=["--name", "-n"],
                help="name of the new {} app".format(app_type),
                local_context_attribute=LocalContextAttribute(
                    name=scope + "_name", actions=[LocalContextAction.SET], scopes=[scope]
                ),
            )
            c.argument(
                "storage_account",
            ...
            c.argument(
                ...
                help="Geographic location where {} app will be hosted. Use `az {} list-consumption-locations` to view available locations.".format(
                    app_type, scope
                ),
            )
            ...
            c.argument(
                "app_insights",
                help="Name of the existing App Insights project to be added to the {} app. Must be in the ".format(
                    app_type
                )
                + "same resource group.",
            )
            c.argument(
                ...
                arg_type=get_three_state_flag(return_label=True),
                help="Disable creating application insights resource during {} create. No logs will be available.".format(
                    scope
                ),
            )
            c.argument(
                "docker_registry_server_user",
                options_list=["--docker-registry-server-user", "-d"],
                help="The container registry server username.",
            )
            c.argument(
                "docker_registry_server_password",
            ...
            if scope == "functionapp":
                c.argument(
                    "functions_version",
                    help="The functions app version. NOTE: This will be required starting the next release cycle",
                    arg_type=get_enum_type(FUNCTIONS_VERSIONS),
                )
                c.argument(
                    "runtime",
                    help='The functions runtime stack. Use "az functionapp list-runtimes" to check supported runtimes and versions',
                )
                c.argument(
                    "runtime_version",
    ...
    with self.argument_context("functionapp plan") as c:
        ...
        c.argument(
            "number_of_workers",
            options_list=["--number-of-workers", "--min-instances"],
            help="The number of workers for the app service plan.",
        )
        c.argument("max_burst", help="The maximum number of elastic workers for the plan.")
        c.argument("tags", arg_type=tags_type)
    ...
    with self.argument_context("functionapp update") as c:
        ...
        c.argument(
            "force",
            required=False,
            help="Required if attempting to migrate functionapp from Premium to Consumption --plan.",
            action="store_true",
        )
    ...
    with self.argument_context("functionapp plan create") as c:
        ...
        c.argument(
            "sku",
            required=True,
            help="The SKU of the app service plan. e.g., F1(Free), D1(Shared), B1(Basic Small), "
    ...
    with self.argument_context("functionapp deployment slot") as c:
        ...
        c.argument(
            "webapp",
            arg_type=functionapp_name_arg_type,
            completer=get_resource_name_completion_list("Microsoft.Web/sites"),
            help="Name of the function app",
            id_part="name",
        )
    ...
    for scope in ["webapp", "functionapp"]:
        ...
        with self.argument_context(scope + " config access-restriction add") as c:
            c.argument("name", arg_type=(webapp_name_arg_type if scope == "webapp" else functionapp_name_arg_type))
            c.argument(
                "rule_name", options_list=["--rule-name", "-r"], help="Name of the access restriction rule to add"
            )
            c.argument("priority", options_list=["--priority", "-p"], help="Priority of the access restriction rule")
            c.argument("description", help="Description of the access restriction rule")
            c.argument("action", arg_type=get_enum_type(ACCESS_RESTRICTION_ACTION_TYPES), help="Allow or deny access")
            c.argument(
                "ip_address",
                help="IP address or CIDR range (optional comma separated list of up to 8 ranges)",
                validator=validate_ip_address,
            )
            c.argument(
                "service_tag",
                help="Service Tag (optional comma separated list of up to 8 tags)",
            ...
            c.argument(
                "scm_site", help="True if access restrictions is added for scm site", arg_type=get_three_state_flag()
            )
            c.argument(
                "vnet_resource_group", help="Resource group of virtual network (default is web app resource group)"
            )
        ...
        with self.argument_context(scope + " config access-restriction remove") as c:
            c.argument("name", arg_type=(webapp_name_arg_type if scope == "webapp" else functionapp_name_arg_type))
            c.argument("rule_name", options_list=["--rule-name", "-r"], help="Name of the access restriction to remove")
            c.argument(
                "ip_address",
                help="IP address or CIDR range (optional comma separated list of up to 8 ranges)",
                validator=validate_ip_address,
            ...
            c.argument(
                "scm_site",
                help="True if access restriction should be removed from scm site",
                arg_type=get_three_state_flag(),
            )
            c.argument("action", arg_type=get_enum_type(ACCESS_RESTRICTION_ACTION_TYPES), help="Allow or deny access")
    ...
    with self.argument_context("appservice ase create") as c:
        c.argument(
            "name",
            options_list=["--name", "-n"],
            validator=validate_ase_create,
            help="Name of the app service environment",
            local_context_attribute=LocalContextAttribute(
                name="ase_name", actions=[LocalContextAction.SET], scopes=["appservice"]
            ),
        )
        c.argument(
            "kind",
            options_list=["--kind", "-k"],
            arg_type=get_enum_type(ASE_KINDS),
            default="ASEv2",
            help="Specify App Service Environment version",
        )
        c.argument(
            "subnet",
            help="Name or ID of existing subnet. To create vnet and/or subnet \
                   use `az network vnet [subnet] create`",
        )
        c.argument("vnet_name", help="Name of the vNet. Mandatory if only subnet name is specified.")
        c.argument(
            "virtual_ip_type",
            arg_type=get_enum_type(ASE_LOADBALANCER_MODES),
            help="Specify if app service environment should be accessible from internet",
        )
        c.argument(
            "ignore_subnet_size_validation",
            arg_type=get_three_state_flag(),
        ...
        c.argument(
            ...
            type=int,
            validator=validate_front_end_scale_factor,
            help="Scale of front ends to app service plan instance ratio. Applies to ASEv2 only.",
            default=15,
        )
        c.argument(
            "front_end_sku",
            arg_type=isolated_sku_arg_type,
            default="I1",
            help="Size of front end servers. Applies to ASEv2 only.",
        )
        c.argument(
            "os_preference",
            arg_type=get_enum_type(ASE_OS_PREFERENCE_TYPES),
    ...
    with self.argument_context("appservice ase update") as c:
        ...
        c.argument("front_end_sku", arg_type=isolated_sku_arg_type, help="(ASEv2 only) Size of front end servers.")
        c.argument(
    ...
    with self.argument_context("appservice ase create-inbound-services") as c:
        ...
        c.argument(
            "subnet",
            help="Name or ID of existing subnet for inbound traffic to ASEv3. \
                   To create vnet and/or subnet use `az network vnet [subnet] create`",
        )
        c.argument("vnet_name", help="Name of the vNet. Mandatory if only subnet name is specified.")
        c.argument("skip_dns", arg_type=get_three_state_flag(), help="Do not create Private DNS Zone and DNS records.")
    ...
    with self.argument_context("appservice domain create") as c:
        ...
        c.argument(
            "contact_info",
            options_list=["--contact-info", "-c"],
            help="The file path to a JSON object with your contact info for domain registration. "
        ...
        c.argument(
            "accept_terms",
            options_list=["--accept-terms"],
            help="By using this flag, you are accepting "
        ...
        c.argument(
            "no_wait", help="Do not wait for the create to complete, and return immediately after queuing the create."
        )
    ...
    with self.argument_context("staticwebapp environment") as c:
        c.argument(
            "environment_name", options_list=["--environment-name"], help="Name of the environment of static site"
        )
    ...
    with self.argument_context("staticwebapp appsettings") as c:
        c.argument(
            "setting_pairs",
            options_list=["--setting-names"],
            help="Space-separated app settings in 'key=value' format. ",
            nargs="*",
        )
        c.argument(
            "setting_names", options_list=["--setting-names"], help="Space-separated app setting names.", nargs="*"
        )
    with self.argument_context("staticwebapp users") as c:
        c.argument(
            "authentication_provider",
            options_list=["--authentication-provider"],
            help="Authentication provider of the user identity such as AAD, Facebook, GitHub, Google, Twitter.",
        )
        c.argument(
            "user_details",
            options_list=["--user-details"],
            help="Email for AAD, Facebook, and Google. Account name (handle) for GitHub and Twitter.",
        )
        c.argument("user_id", help="Given id of registered user.")
        c.argument("domain", options_list=["--domain"], help="A domain added to the static app in quotes.")
        c.argument(
            "roles",
            options_list=["--roles"],
        ...
        c.argument(
            ...
            options_list=["--invitation-expiration-in-hours"],
    ...
    with self.argument_context("staticwebapp functions link") as c:
        c.argument(
            ...
            help="Resource ID of the functionapp to link. Can be retrieved with 'az functionapp --query id'",
        )
        c.argument(
            "force",
            help="Force the function link even if the function is already linked to a static webapp. May be needed if the function was previously linked to a static webapp.",
        )
