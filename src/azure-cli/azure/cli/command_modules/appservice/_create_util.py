...
from ._constants import (
    ...
    OS_DEFAULT,
    DOTNET_RUNTIME_NAME,
    DOTNET_TARGET_FRAMEWORK_REGEX,
    GENERATE_RANDOM_APP_NAMES,
)
...
def zip_contents_from_dir(dirPath, lang):
    ...
    try:
        ...
        if e.errno == 13:
            raise CLIError(
                "Insufficient permissions to create a zip in current directory. "
...
def get_lang_from_content(src_path, html=False, is_linux=False):
    ...
    for _dirpath, _dirnames, files in os.walk(src_path):
        for file in files:
            if html and (
                fnmatch.fnmatch(file, "*.html") or fnmatch.fnmatch(file, "*.htm") or fnmatch.fnmatch(file, "*shtml.")
            ):
    ...
    if html:
        if static_html_file:
            ...
            raise CLIError(
                "The html flag was passed, but could not find HTML files, "
...
def get_site_availability(cmd, name):
    ...
    if "." in name:
        ...
        availability.message = (
            "Site names only allow alphanumeric characters and hyphens, "
...
def detect_os_form_src(src_dir, html=False):
    ...
    return (
        "Linux"
        if language is not None and language.lower() == NODE_RUNTIME_NAME or language.lower() == PYTHON_RUNTIME_NAME
        else OS_DEFAULT
    )
...
def get_plan_to_use(cmd, user, loc, sku, create_rg, resource_group_name, client, is_linux=False, plan=None):
    ...
    if asp is not None:
        if asp.reserved != is_linux:
            logger.warning(
                "Existing app service plan %s has a different OS type and deployment may fail. "
                "Consider using a different OS or app service plan",
                plan,
            )
    return plan
...
def _determine_if_default_plan_to_use(cmd, plan_name, resource_group_name, loc, sku, create_rg, is_linux):
    ...
    _asp_list = list(
        filter(lambda x: _asp_generic in x.name, client.app_service_plans.list_by_resource_group(resource_group_name))
    )
    ...
    if _num_asp:
        ...
        selected_asp = next(
            (
                a
                for a in _asp_list
                if isinstance(a.sku, SkuDescription)
                and a.sku.name.lower() == sku.lower()
                and (a.location.replace(" ", "").lower() == loc.lower())
                and a.reserved == is_linux
            ),
            None,
        )
