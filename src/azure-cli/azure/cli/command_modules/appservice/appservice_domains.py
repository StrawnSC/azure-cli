...
def create_domain(
    ...
    if not hostname_availability.available:
        raise ValidationError(
            "Custom domain name '{}' is not available. Please try again " "with a new hostname.".format(hostname)
        )
...
def show_domain_purchase_terms(cmd, hostname):
    ...
    terms = {
        ...
        "legal_terms": "https://storedomainslegalterms.blob.core.windows.net/domain-purchase-legal-terms/legal_terms.txt",
...
def verify_contact_info_and_format(contact_info):
    ...
    for required_key in required_keys:
        if not (
            required_key in contact_info
            and "value" in contact_info[required_key]
            and contact_info[required_key]["value"]
        ):  # pylint: disable=line-too-long
    ...
    if not re.match(_phone_regex, contact_info["phone"]["value"]):
        raise CLIError(
            "Invalid value: phone number must match pattern +areacode.phonenumber, " 'for example "+1.0000000000"'
        )
    ...
    if "fax" in contact_info and "value" in contact_info["fax"] and contact_info["fax"]["value"]:
        if not re.match(_phone_regex, contact_info["fax"]["value"]):
            raise CLIError(
                "Invalid value: fax number must match pattern +areacode.phonenumber, " 'for example "+1.0000000000"'
            )
    ...
    if (
        "name_middle" in contact_info
        and "value" in contact_info["name_middle"]
        and contact_info["name_middle"]["value"]
    ):  # pylint: disable=line-too-long
    ...
    if (
        "organization" in contact_info
        and "value" in contact_info["organization"]
        and contact_info["organization"]["value"]
    ):  # pylint: disable=line-too-long
