...
from ._create_util import (
    ...
    get_sku_to_use,
    detect_os_form_src,
    get_current_stack_from_runtime,
    generate_default_app_name,
)
from ._constants import (
    FUNCTIONS_STACKS_API_KEYS,
    FUNCTIONS_LINUX_RUNTIME_VERSION_REGEX,
    FUNCTIONS_WINDOWS_RUNTIME_VERSION_REGEX,
...
def create_webapp(
    ...
    deployment_source_url=None,
    deployment_source_branch="master",
    deployment_local_git=None,
    docker_registry_server_password=None,
    docker_registry_server_user=None,
    multicontainer_config_type=None,
    multicontainer_config_file=None,
    tags=None,
    using_webapp_up=False,
    language=None,
    ...
    SiteConfig, SkuDescription, NameValuePair = cmd.get_models("SiteConfig", "SkuDescription", "NameValuePair")
    ...
    if not plan_info:
        raise ResourceNotFoundError(
            "The plan '{}' doesn't exist in the resource group '{}".format(plan, resource_group_name)
        )
    ...
    if not name_validation.name_available:
        ...
        if app_details is None:
            raise ResourceNotFoundError(
                "Unable to retrieve details of the existing app '{}'. Please check that "
                "the app is a part of the current subscription".format(name)
            )
        current_rg = app_details.resource_group
        if resource_group_name is not None and (resource_group_name.lower() != current_rg.lower()):
            raise ValidationError(
                "The webapp '{}' exists in resource group '{}' and does not "
                "match the value entered '{}'. Please re-run command with the "
                "correct parameters.".format(name, current_rg, resource_group_name)
            )
        existing_app_settings = _generic_site_operation(
            cmd.cli_ctx, resource_group_name, name, "list_application_settings"
        )
        settings = []
    ...
    if isinstance(plan_info.sku, SkuDescription) and plan_info.sku.name.upper() not in [
        "F1",
        "FREE",
        "SHARED",
        "D1",
    ...
    if subnet or vnet:
        subnet_info = _get_subnet_info(cmd=cmd, resource_group_name=resource_group_name, subnet=subnet, vnet=vnet)
        _validate_vnet_integration_location(
            cmd=cmd,
        ...
        _vnet_delegation_check(
            cmd,
            subnet_subscription_id=subnet_info["subnet_subscription_id"],
    ...
    current_stack = None
    if is_linux:
        if not validate_container_app_create_options(
            runtime, deployment_container_image_name, multicontainer_config_type, multicontainer_config_file
        ):
            raise ArgumentUsageError(
                "usage error: --runtime | --deployment-container-image-name |"
        ...
        if runtime:
            ...
            if not match:
                raise ValidationError(
                    "Linux Runtime '{}' is not supported."
        ...
        if name_validation.name_available:
            site_config.app_settings.append(
                NameValuePair(name="DOCKER_REGISTRY_SERVER_USERNAME", value=docker_registry_server_user)
            )
            site_config.app_settings.append(
                NameValuePair(name="DOCKER_REGISTRY_SERVER_PASSWORD", value=docker_registry_server_password)
            )
            site_config.app_settings.append(
        ...
        if any([startup_file, deployment_container_image_name, multicontainer_config_file, multicontainer_config_type]):
            raise ArgumentUsageError(
                "usage error: --startup-file or --deployment-container-image-name or "
        ...
        if name_validation.name_available:  # If creating new webapp
            ...
            site_config.app_settings.append(
                NameValuePair(name="WEBSITE_NODE_DEFAULT_VERSION", value=node_default_version)
            )
    ...
    _set_remote_or_local_git(
        cmd, webapp, resource_group_name, name, deployment_source_url, deployment_source_branch, deployment_local_git
    )
    ...
    if assign_identities is not None:
        identity = assign_identity(cmd, resource_group_name, name, assign_identities, role, None, scope)
...
def _validate_vnet_integration_location(cmd, subnet_resource_group, vnet_name, webapp_location, vnet_sub_id=None):
    ...
    vnet_location = vnet_client.get(resource_group_name=subnet_resource_group, virtual_network_name=vnet_name).location
    ...
    if vnet_location != asp_location:
        raise ArgumentUsageError(
            "Unable to create webapp: vnet and App Service Plan must be in the same location. "
...
def _get_subnet_info(cmd, resource_group_name, vnet, subnet):
    ...
    if is_valid_resource_id(subnet):
        ...
        subnet_info["vnet_resource_id"] = vnet_fmt.format(
    ...
    if is_valid_resource_id(vnet):
        ...
        logger.warning(
            "Assuming subnet resource group is the same as webapp. "
        ...
        vnet_fmt = "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/virtualNetworks/{}"
        subnet_info["vnet_resource_id"] = vnet_fmt.format(subscription_id, subnet_rg, vnet)
...
def validate_container_app_create_options(
    runtime=None, deployment_container_image_name=None, multicontainer_config_type=None, multicontainer_config_file=None
):
...
def update_app_settings(cmd, resource_group_name, name, settings=None, slot=None, slot_settings=None):
    ...
    result = _generic_settings_operation(
        cmd.cli_ctx, resource_group_name, name, "update_application_settings", app_settings, slot, client
    )
...
def add_azure_storage_account(
    ...
    azure_storage_accounts = _generic_site_operation(
        cmd.cli_ctx, resource_group_name, name, "list_azure_storage_accounts", slot
    )

    if custom_id in azure_storage_accounts.properties:
        raise CLIError(
            "Site already configured with an Azure storage account with the id '{}'. "
            "Use 'az webapp config storage-account update' to update an existing "
            "Azure storage account configuration.".format(custom_id)
        )

    azure_storage_accounts.properties[custom_id] = AzureStorageInfoValue(
        type=storage_type,
    ...
    result = _generic_settings_operation(
        cmd.cli_ctx, resource_group_name, name, "update_azure_storage_accounts", azure_storage_accounts, slot, client
    )
...
def update_azure_storage_account(
    ...
    if not existing_account_config:
        raise CLIError(
            "No Azure storage account configuration found with the id '{}'. "
...
def enable_zip_deploy(cmd, resource_group_name, name, src, timeout=None, slot=None):
    ...
    if res.status_code == 202:
        response = _check_zip_deployment_status(
            cmd, resource_group_name, name, deployment_status_url, authorization, timeout
        )
        return response
    ...
    if res.status_code == 409:
        raise UnclassifiedUserFault(
            "There may be an ongoing deployment or your app setting has "
    ...
    if res.status_code:
        raise AzureInternalError(
            "An error occured during deployment. Status Code: {}, Details: {}".format(res.status_code, res.text)
        )
...
def add_remote_build_app_settings(cmd, resource_group_name, name, slot):
    ...
    if scm_do_build_during_deployment is not True:
        ...
        update_app_settings(cmd, resource_group_name, name, ["SCM_DO_BUILD_DURING_DEPLOYMENT=true"], slot)
    ...
    if website_run_from_package:
        ...
        delete_app_settings(cmd, resource_group_name, name, ["WEBSITE_RUN_FROM_PACKAGE"], slot)
    ...
    if enable_oryx_build:
        ...
        delete_app_settings(cmd, resource_group_name, name, ["ENABLE_ORYX_BUILD"], slot)
...
def remove_remote_build_app_settings(cmd, resource_group_name, name, slot):
    ...
    if scm_do_build_during_deployment is not False:
        ...
        update_app_settings(cmd, resource_group_name, name, ["SCM_DO_BUILD_DURING_DEPLOYMENT=false"], slot)
    ...
    if app_settings_should_contain:
        ...
        while not scm_is_up_to_date and retries >= 0:
            scm_is_up_to_date = validate_app_settings_in_scm(
                cmd, resource_group_name, name, slot, should_contain=app_settings_should_contain
            )
            retries -= 1
            time.sleep(5)
...
def upload_zip_to_storage(cmd, resource_group_name, name, src, slot=None):
    ...
    block_blob_service.create_blob_from_path(
        container_name, blob_name, src, validate_content=True, progress_callback=progress_callback
    )
    ...
    blob_token = block_blob_service.generate_blob_shared_access_signature(
        container_name, blob_name, permission=BlobPermissions(read=True), expiry=blob_end, start=blob_start
    )
...
def update_webapp(
    ...
    if minimum_elastic_instance_count or prewarmed_instance_count:
        ...
        if (minimum_elastic_instance_count or 0) > plan.maximum_elastic_worker_count:
            raise ValidationError(
                "--minimum-elastic-instance-count: Minimum elastic instance count is greater than "
        ...
        if (prewarmed_instance_count or 0) > plan.maximum_elastic_worker_count:
            raise ValidationError(
                "--prewarmed-instance-count: Prewarmed instance count is greater than "
                "the app service plan's maximum Elastic worker count. "
                "Please choose a lower count or update the plan's maximum "
            )
    ...
    if plan is not None:
        if is_valid_resource_id(plan):
            ...
            dest_plan_info = client.app_service_plans.get(
                dest_parse_result["resource_group"], dest_parse_result["name"]
            )
        else:
...
def validate_plan_switch_compatibility(cmd, client, src_functionapp_instance, dest_plan_instance, force):
    ...
    src_plan_info = client.app_service_plans.get(src_parse_result["resource_group"], src_parse_result["name"])
    ...
    if src_plan_info.reserved or dest_plan_instance.reserved:
        raise ValidationError(
            "This feature currently supports windows to windows plan migrations. For other "
    ...
    if not (dest_is_consumption or is_plan_elastic_premium(cmd, dest_plan_instance)):
        raise ValidationError(
            "You are trying to move to a plan that is not a Consumption or an "
            "Elastic Premium plan. " + general_switch_msg
        )

    if src_is_premium and dest_is_consumption:
        logger.warning(
            "WARNING: Moving a functionapp from Premium to Consumption might result in loss of "
        ...
        if not force:
            raise RequiredArgumentMissingError(
                "If you want to migrate a functionapp from a Premium to Consumption "
...
def restore_deleted_webapp(cmd, deleted_id, resource_group_name, name, slot=None, restore_content_only=None):
    ...
    return _generic_site_operation(
        cmd.cli_ctx, resource_group_name, name, "begin_restore_from_deleted_app", slot, request
    )
...
def list_function_app(cmd, resource_group_name=None):
    return list(
        filter(
            lambda x: x.kind is not None and "function" in x.kind.lower(), _list_app(cmd.cli_ctx, resource_group_name)
        )
    )
...
def _show_app(cmd, resource_group_name, name, cmd_app_type, slot=None):
    ...
    if not app:
        raise ResourceNotFoundError(
            "Unable to find {} '{}', in RG '{}'.".format(cmd_app_type, name, resource_group_name)
        )
    ...
    app.site_config = _generic_site_operation(
        cmd.cli_ctx, resource_group_name, name, "get_configuration", slot, api_version="2021-01-15"
    )
...
def assign_identity(cmd, resource_group_name, name, assign_identities=None, role="Contributor", slot=None, scope=None):
    ManagedServiceIdentity, ResourceIdentityType = cmd.get_models(
    ...
    def setter(webapp):
        ...
        poller = _generic_site_operation(
            cmd.cli_ctx, resource_group_name, name, "begin_create_or_update", extra_parameter=webapp, slot=slot
        )
    ...
    def setter(webapp):
        ...
        if remove_local_identity:
            webapp.identity.type = (
                IdentityType.none
                if webapp.identity.type == IdentityType.system_assigned or webapp.identity.type == IdentityType.none
...
def delete_webapp(
    cmd, resource_group_name, name, keep_metrics=None, keep_empty_plan=None, keep_dns_registration=None, slot=None
):  # pylint: disable=unused-argument
    client = web_client_factory(cmd.cli_ctx)
    if slot:
...
def validate_app_settings_in_scm(
    cmd, resource_group_name, name, slot=None, should_have=None, should_not_have=None, should_contain=None
):
...
def get_connection_strings(cmd, resource_group_name, name, slot=None):
    ...
    client = web_client_factory(cmd.cli_ctx)
    slot_constr_names = (
        client.web_apps.list_slot_configuration_names(resource_group_name, name).connection_string_names or []
    )
    result = [
        {
            "name": p,
            ...
            "slotSetting": p in slot_constr_names,
        }
...
def get_azure_storage_accounts(cmd, resource_group_name, name, slot=None):
    client = web_client_factory(cmd.cli_ctx)
    result = _generic_site_operation(cmd.cli_ctx, resource_group_name, name, "list_azure_storage_accounts", slot)

    slot_azure_storage_config_names = (
        client.web_apps.list_slot_configuration_names(resource_group_name, name).azure_storage_config_names or []
    )

    return [
        {"name": p, "value": result.properties[p], "slotSetting": p in slot_azure_storage_config_names}
        for p in result.properties
    ]
...
def _add_fx_version(cmd, resource_group_name, name, custom_image_name, slot=None):
    ...
    return update_site_configs(
        cmd, resource_group_name, name, linux_fx_version=linux_fx, windows_fx_version=windows_fx, slot=slot
    )
...
def _get_linux_multicontainer_decoded_config(cmd, resource_group_name, name, slot=None):
    ...
    if not any(linux_fx_version.startswith(s) for s in MULTI_CONTAINER_TYPES):
        raise CLIError(
            "Cannot decode config that is not one of the" " following types: {}".format(",".join(MULTI_CONTAINER_TYPES))
        )
...
def update_site_configs(
    ...
    pre_warmed_instance_count=None,
    php_version=None,
    python_version=None,
    net_framework_version=None,
    java_version=None,
    java_container=None,
    java_container_version=None,
    remote_debugging_enabled=None,
    web_sockets_enabled=None,
    always_on=None,
    auto_heal_enabled=None,
    ...
    if pre_warmed_instance_count is not None:
        pre_warmed_instance_count = validate_range_of_int_flag(
            "--prewarmed-instance-count", pre_warmed_instance_count, min_val=0, max_val=20
        )
    import inspect
...
def _build_app_settings_output(app_settings, slot_cfg_names):
    ...
    return [
        {"name": p, "value": app_settings[p], "slotSetting": p in slot_cfg_names}
        for p in _mask_creds_related_appsettings(app_settings)
    ]


def update_connection_strings(
    cmd, resource_group_name, name, connection_string_type, settings=None, slot=None, slot_settings=None
):
    ...
    conn_strings = _generic_site_operation(cmd.cli_ctx, resource_group_name, name, "list_connection_strings", slot)
    for name_value in settings + slot_settings:
        ...
        if value[0] in ["'", '"']:  # strip away the quots used as separators
            value = value[1:-1]
        conn_strings.properties[conn_string_name] = ConnStringValueTypePair(value=value, type=connection_string_type)
    client = web_client_factory(cmd.cli_ctx)
    result = _generic_settings_operation(
        cmd.cli_ctx, resource_group_name, name, "update_connection_strings", conn_strings, slot, client
    )
...
def delete_connection_strings(cmd, resource_group_name, name, setting_names, slot=None):
    ...
    return _generic_settings_operation(
        cmd.cli_ctx, resource_group_name, name, "update_connection_strings", conn_strings, slot, client
    )


CONTAINER_APPSETTING_NAMES = [
    "DOCKER_REGISTRY_SERVER_URL",
...
def update_container_settings(
    ...
    docker_registry_server_user=None,
    websites_enable_app_service_storage=None,
    docker_registry_server_password=None,
    multicontainer_config_type=None,
    multicontainer_config_file=None,
    slot=None,
    ...
    if (
        not docker_registry_server_user
        and not docker_registry_server_password
        and docker_registry_server_url
        and ".azurecr.io" in docker_registry_server_url
    ):
    ...
    if (
        docker_registry_server_user
        or docker_registry_server_password
        or docker_registry_server_url
        or websites_enable_app_service_storage
    ):  # pylint: disable=line-too-long
    ...
    return _mask_creds_related_appsettings(
        _filter_for_container_settings(cmd, resource_group_name, name, settings, slot=slot)
    )


def update_container_settings_functionapp(
    cmd,
    resource_group_name,
    name,
    docker_registry_server_url=None,
    ...
    return update_container_settings(
        cmd,
        resource_group_name,
        name,
        docker_registry_server_url,
...
def show_container_settings(cmd, resource_group_name, name, show_multicontainer_config=None, slot=None):
    settings = get_app_settings(cmd, resource_group_name, name, slot)
    return _mask_creds_related_appsettings(
        _filter_for_container_settings(cmd, resource_group_name, name, settings, show_multicontainer_config, slot)
    )
...
def _filter_for_container_settings(
    cmd, resource_group_name, name, settings, show_multicontainer_config=None, slot=None
):
    result = [x for x in settings if x["name"] in CONTAINER_APPSETTING_NAMES]
    fx_version = _get_fx_version(cmd, resource_group_name, name, slot).strip()
    if fx_version:
        added_image_name = {"name": "DOCKER_CUSTOM_IMAGE_NAME", "value": fx_version}
        ...
        if show_multicontainer_config:
            ...
            decoded_image_name = {"name": "DOCKER_CUSTOM_IMAGE_NAME_DECODED", "value": decoded_value}
...
def add_hostname(cmd, resource_group_name, webapp_name, hostname, slot=None):
    ...
    if slot is None:
        return client.web_apps.create_or_update_host_name_binding(
            resource_group_name=resource_group_name, name=webapp.name, host_name=hostname, host_name_binding=binding
        )
...
def list_hostnames(cmd, resource_group_name, webapp_name, slot=None):
    result = list(
        _generic_site_operation(cmd.cli_ctx, resource_group_name, webapp_name, "list_host_name_bindings", slot)
    )
    for r in result:
...
def create_webapp_slot(cmd, resource_group_name, webapp, slot, configuration_source=None):
    ...
    if configuration_source and site_config.windows_fx_version:
        ...
        app_settings = _generic_site_operation(
            cmd.cli_ctx, resource_group_name, webapp, "list_application_settings", src_slot
        )
        settings = []
...
def update_slot_configuration_from_source(cmd, client, resource_group_name, webapp, slot, configuration_source=None):
    clone_from_prod = configuration_source.lower() == webapp.lower()
    site_config = get_site_configs(cmd, resource_group_name, webapp, None if clone_from_prod else configuration_source)
    _generic_site_operation(cmd.cli_ctx, resource_group_name, webapp, "update_configuration", slot, site_config)
    ...
    connection_strings = _generic_site_operation(
        cmd.cli_ctx, resource_group_name, webapp, "list_connection_strings", src_slot
    )
    for a in slot_cfg_names.connection_string_names or []:
        connection_strings.properties.pop(a, None)

    _generic_settings_operation(
        cmd.cli_ctx, resource_group_name, webapp, "update_application_settings", app_settings, slot, client
    )
    _generic_settings_operation(
        cmd.cli_ctx, resource_group_name, webapp, "update_connection_strings", connection_strings, slot, client
    )


def config_source_control(
    cmd,
    resource_group_name,
    name,
    repo_url,
    repository_type="git",
    branch=None,  # pylint: disable=too-many-locals
    ...
    source_control = SiteSourceControl(
        ...
        is_git_hub_action=bool(github_action),
    )
    ...
    for i in range(5):
        try:
            poller = _generic_site_operation(
                cmd.cli_ctx, resource_group_name, name, "begin_create_or_update_source_control", slot, source_control
            )
            return LongRunningOperation(cmd.cli_ctx)(poller)
        except Exception as ex:  # pylint: disable=broad-except
            import re
...
def create_app_service_plan(
    ...
    plan_def = AppServicePlan(
        ...
        hyper_v=(hyper_v or None),
        name=name,
        per_site_scaling=per_site_scaling,
        hosting_environment_profile=ase_def,
    )

    if sku.upper() in ["WS1", "WS2", "WS3"]:
        existing_plan = get_resource_if_exists(
            client.app_service_plans, resource_group_name=resource_group_name, name=name
        )
...
def update_app_service_plan(
    instance, sku=None, number_of_workers=None, elastic_scale=None, max_elastic_worker_count=None
):
    ...
    if elastic_scale is not None or max_elastic_worker_count is not None:
        ...
        if get_sku_tier(sku) not in ["PREMIUMV2", "PREMIUMV3"]:
            raise ValidationError(
                "--number-of-workers and --elastic-scale can only be used on premium V2/V3 SKUs. "
    ...
    if max_elastic_worker_count is not None:
        ...
        if max_elastic_worker_count < number_of_workers:
            raise InvalidArgumentValueError(
                "--max-elastic-worker-count must be greater than or equal to the "
...
def update_functionapp_app_service_plan(cmd, instance, sku=None, number_of_workers=None, max_burst=None):
    ...
    if number_of_workers is not None:
        number_of_workers = validate_range_of_int_flag(
            "--number-of-workers / --min-instances", number_of_workers, min_val=0, max_val=20
        )
...
def create_backup(
    ...
    backup_request = BackupRequest(
        backup_name=backup_name, storage_account_url=storage_account_url, databases=db_setting
    )
    if slot:
...
def update_backup_schedule(
    ...
    try:
        configuration = _generic_site_operation(
            cmd.cli_ctx, resource_group_name, webapp_name, "get_backup_configuration", slot
        )
    except Exception:  # pylint: disable=broad-except
        # No configuration set yet
        if not all([storage_account_url, frequency, retention_period_in_days, keep_at_least_one_backup]):
            raise CLIError(
                "No backup configuration found. A configuration must be created. "
    ...
    backup_schedule = BackupSchedule(
        frequency_interval=frequency_num,
        frequency_unit=frequency_unit.name,
        keep_at_least_one_backup=keep_at_least_one_backup,
        retention_period_in_days=retention_period_in_days,
    )
    backup_request = BackupRequest(
        backup_request_name=backup_name,
    ...
    return _generic_site_operation(
        cmd.cli_ctx, resource_group_name, webapp_name, "update_backup_configuration", slot, backup_request
    )
...
def list_snapshots(cmd, resource_group_name, name, slot=None):
    return _generic_site_operation(cmd.cli_ctx, resource_group_name, name, "list_snapshots", slot)
...
def restore_snapshot(
    ...
    if all([source_resource_group, source_name]):
        ...
        request = SnapshotRestoreRequest(
            overwrite=False, snapshot_time=time, recovery_source=source, recover_configuration=recover_config
        )
        if slot:
...
def _get_local_git_url(cli_ctx, client, resource_group_name, name, slot=None):
    ...
    return "{}://{}@{}/{}.git".format(parsed.scheme, user.publishing_user_name, parsed.netloc, name)
...
def list_publishing_credentials(cmd, resource_group_name, name, slot=None):
    content = _generic_site_operation(cmd.cli_ctx, resource_group_name, name, "begin_list_publishing_credentials", slot)
...
def list_publish_profiles(cmd, resource_group_name, name, slot=None, xml=False):
    ...
    content = _generic_site_operation(
        cmd.cli_ctx,
        resource_group_name,
        name,
...
def _get_url(cmd, resource_group_name, name, slot=None):
    ...
    ssl_host = next((h for h in site.host_name_ssl_states if h.ssl_state != SslState.disabled), None)
...
def config_diagnostics(
    ...
    if application_logging:
        ...
        if application_logging in ["azureblobstorage", "off"]:
            blob_log = AzureBlobStorageApplicationLogsConfig(level=level, retention_in_days=3, sas_url=None)
        application_logs = ApplicationLogsConfig(file_system=fs_log, azure_blob_storage=blob_log)
    ...
    detailed_error_messages_logs = (
        None if detailed_error_messages is None else EnabledConfig(enabled=detailed_error_messages)
    )
    failed_request_tracing_logs = (
        None if failed_request_tracing is None else EnabledConfig(enabled=failed_request_tracing)
    )
    ...
    return _generic_site_operation(
        cmd.cli_ctx, resource_group_name, name, "update_diagnostic_logs_config", slot, site_log_config
    )
...
def show_deployment_log(cmd, resource_group, name, slot=None, deployment_id=None):
    ...
    if deployment_id:
        ...
        sorted_logs = sorted(response.json(), key=lambda x: x["start_time"], reverse=True)
...
def set_traffic_routing(cmd, resource_group_name, name, distribution):
    ...
    for r in distribution:
        ...
        configs.experiments.ramp_up_rules.append(
            RampUpRule(
                action_host_name=action_host_name_slot + host_name_suffix,
...
def _get_log(url, user_name, password, log_file=None):
    ...
    if r.status != 200:
        raise CLIError(
            "Failed to connect to '{}' with status code '{}' and reason '{}'".format(url, r.status, r.reason)
        )
    if log_file:  # download logs
        ...
        for chunk in r.stream():
            if chunk:
                ...
                logger.warning(
                    chunk.decode(encoding="utf-8", errors="replace")
                    .encode(std_encoding, errors="replace")
                    .decode(std_encoding, errors="replace")
                    .rstrip("\n\r")
                )  # each line of log has CRLF.
    r.release_conn()
...
def upload_ssl_cert(cmd, resource_group_name, name, certificate_password, certificate_file, slot=None):
    ...
    hosting_environment_profile_param = (
        webapp.hosting_environment_profile.name if webapp.hosting_environment_profile else ""
    )

    thumb_print = _get_cert(certificate_password, certificate_file)
    cert_name = _generate_cert_name(
        thumb_print, hosting_environment_profile_param, webapp.location, resource_group_name
    )
    cert = Certificate(
        password=certificate_password,
        pfx_blob=cert_contents,
...
def import_ssl_cert(cmd, resource_group_name, name, key_vault, key_vault_certificate_name):
    ...
    if kv_id is None:
        kv_msg = (
            "The Key Vault {0} was not found in the subscription in context. "
    ...
    if cloud_type.lower() == PUBLIC_CLOUD.lower():
        if kv_subscription.lower() != subscription_id.lower():
            diff_subscription_client = get_mgmt_service_client(
                cmd.cli_ctx, ResourceType.MGMT_APPSERVICE, subscription_id=kv_subscription
            )
    ...
    return client.certificates.create_or_update(
        name=cert_name, resource_group_name=resource_group_name, certificate_envelope=kv_cert_def
    )
...
def create_managed_ssl_cert(cmd, resource_group_name, name, hostname, slot=None):
    ...
    if not _verify_hostname_binding(cmd, resource_group_name, name, hostname, slot):
        ...
        raise CLIError(
            "Hostname (custom domain) '{0}' is not registered with {1}. "
    ...
    server_farm_id = webapp.server_farm_id
    location = webapp.location
    easy_cert_def = Certificate(location=location, canonical_name=hostname, server_farm_id=server_farm_id, password="")

    # TODO: Update manual polling to use LongRunningOperation once backend API & new SDK supports polling
    try:
        return client.certificates.create_or_update(
            name=hostname, resource_group_name=resource_group_name, certificate_envelope=easy_cert_def
        )
        ...
        if ex.response.status_code == 202 and poll_url:
            ...
            if r.status_code == 200:
                try:
                    ...
                    return r.text
            logger.warning(
                "Managed Certificate creation in progress. Please use the command "
                "'az webapp config ssl show -g %s --certificate-name %s' "
                " to view your certificate once it is created",
                resource_group_name,
                hostname,
            )
            return
...
def _update_host_name_ssl_state(
    cmd, resource_group_name, webapp_name, webapp, host_name, ssl_state, thumbprint, slot=None
):
    ...
    return _generic_site_operation(
        cmd.cli_ctx, resource_group_name, webapp_name, "begin_create_or_update", slot, updated_webapp
    )
...
def _update_ssl_binding(cmd, resource_group_name, name, certificate_thumbprint, ssl_type, slot=None):
    ...
    if found_cert:
        if len(found_cert.host_names) == 1 and not found_cert.host_names[0].startswith("*"):
            return _update_host_name_ssl_state(
                cmd, resource_group_name, name, webapp, found_cert.host_names[0], ssl_type, certificate_thumbprint, slot
            )
...
def unbind_ssl_cert(cmd, resource_group_name, name, certificate_thumbprint, slot=None):
    SslState = cmd.get_models("SslState")
    return _update_ssl_binding(cmd, resource_group_name, name, certificate_thumbprint, SslState.disabled, slot)
    ...
    def _parse_major_version_windows(self, major_version, parsed_results, config_mappings):
        ...
        if default_java_version:
            ...
            for java_version in ["1.8", "11"]:
                ...
                runtime_name = "{}|{}|{}|{}".format("java", java_version, java_container, container_version)
                gh_actions_version = "8" if java_version == "1.8" else java_version
                gh_actions_runtime = "{}, {}, {}".format(
                    java_version,
                ...
                runtime = self.Runtime(
                    ...
                    github_actions_properties={
                        "github_actions_version": gh_actions_version,
                        "app_runtime": "java",
...
class _FunctionAppStackRuntimeHelper(_AbstractStackRuntimeHelper):
    ...
    def resolve(self, runtime, version=None, functions_version=None, linux=False):
        ...
        if not runtimes:
            ...
            raise ValidationError(
                "Runtime {0} not supported for os {1}. Supported runtimes for os {1} are: {2}. "
                "Run 'az functionapp list-runtimes' for more details on supported runtimes. ".format(
                    runtime, os, supported_runtimes
                )
            )
        if version is None:
        ...
        if not matched_runtime_version:
            ...
            old_to_new_version = {"11": "11.0", "8": "8.0"}
        ...
        if not matched_runtime_version:
            ...
            raise ValidationError(
                "Invalid version: {0} for runtime {1} and os {2}. Supported versions for runtime "
                "{1} and os {2} are: {3}. "
                "Run 'az functionapp list-runtimes' for more details on supported runtimes. ".format(
                    version, runtime, os, versions
                )
            )
        if functions_version not in matched_runtime_version.supported_func_versions:
            ...
            raise ValidationError(
                "Functions version {} is not supported for runtime {} with version {} and os {}. "
                "Supported functions versions are {}. "
                "Run 'az functionapp list-runtimes' for more details on supported runtimes. ".format(
                    functions_version, runtime, version, os, supported_func_versions
                )
            )
    ...
    def get_default_version(self, runtime, functions_version, linux=False):
        ...
        for r in runtimes:
            if functions_version in r.supported_func_versions:
                return r
        raise ValidationError(
            "Could not find a runtime version for runtime {} with functions version {} and os {}"
    ...
    def _parse_minor_version(self, runtime_settings, major_version_name, minor_version_name, runtime_to_version):
        if not runtime_settings.is_deprecated:
            ...
            if functions_versions:
                ...
                runtime_name = (
                    runtime_settings.app_settings_dictionary.get(self.KEYS.FUNCTIONS_WORKER_RUNTIME)
                    or major_version_name
                )
            ...
            for major_version in runtime.major_versions:
                for minor_version in major_version.minor_versions:
                    ...
                    if linux_settings is not None:
                        self._parse_minor_version(
                            runtime_settings=linux_settings,
                    ...
                    if windows_settings is not None:
                        self._parse_minor_version(
                            runtime_settings=windows_settings,
                            major_version_name=runtime.name,
                            minor_version_name=runtime_version,
...
def create_functionapp_app_service_plan(
    cmd,
    resource_group_name,
    name,
    is_linux,
...
def create_functionapp(
    ...
    if functions_version is None:
        logger.warning(
            "No functions version specified so defaulting to 3. In the future, specifying a version will "
    ...
    if vnet or subnet:
        ...
        _validate_vnet_integration_location(
            cmd=cmd,
            webapp_location=webapp_location,
            subnet_resource_group=subnet_info["resource_group_name"],
            vnet_name=subnet_info["vnet_name"],
            vnet_sub_id=subnet_info["subnet_subscription_id"],
        )
    ...
    functionapp_def = Site(
        location=None, site_config=site_config, tags=tags, virtual_network_subnet_id=subnet_resource_id
    )
    ...
    if functions_version == "2" and functionapp_def.location in FUNCTIONS_NO_V2_REGIONS:
        raise ValidationError(
            "2.x functions are not supported in this region. To create a 3.x function, "
    ...
    matched_runtime = runtime_helper.resolve(
        "dotnet" if not runtime else runtime, runtime_version, functions_version, is_linux
    )
    ...
    if is_linux:
        ...
        if not is_consumption:
            site_config.app_settings.append(
                NameValuePair(name="MACHINEKEY_DecryptionKey", value=str(hexlify(urandom(32)).decode()).upper())
            )
            if deployment_container_image_name:
                functionapp_def.kind = "functionapp,linux,container"
                site_config.app_settings.append(
                    NameValuePair(name="DOCKER_CUSTOM_IMAGE_NAME", value=deployment_container_image_name)
                )
    ...
    site_config.app_settings.append(
        NameValuePair(name="FUNCTIONS_EXTENSION_VERSION", value=_get_extension_version_functionapp(functions_version))
    )
    ...
    if is_plan_elastic_premium(cmd, plan_info) or consumption_plan_location is not None:
        site_config.app_settings.append(
            NameValuePair(name="WEBSITE_CONTENTAZUREFILECONNECTIONSTRING", value=con_string)
        )
    ...
    if app_insights_key is not None:
        site_config.app_settings.append(NameValuePair(name="APPINSIGHTS_INSTRUMENTATIONKEY", value=app_insights_key))
    elif app_insights is not None:
        instrumentation_key = get_app_insights_key(cmd.cli_ctx, resource_group_name, app_insights)
        site_config.app_settings.append(NameValuePair(name="APPINSIGHTS_INSTRUMENTATIONKEY", value=instrumentation_key))
    ...
    if consumption_plan_location and is_linux:
        logger.warning(
            "Your Linux function app '%s', that uses a consumption plan has been successfully "
    ...
    if create_app_insights:
        try:
            ...
            logger.warning(
                "Error while trying to create and configure an Application Insights for the Function App. "
                "Please use the Azure Portal to create and configure the Application Insights, if needed."
            )
            update_app_settings(
                cmd, functionapp.resource_group, functionapp.name, ["AzureWebJobsDashboard={}".format(con_string)]
            )
...
def try_create_application_insights(cmd, functionapp):
    creation_failed_warn = (
        "Unable to create the Application Insights for the Function App. "
    ...
    ai_properties = {"name": ai_name, "location": ai_location, "kind": "web", "properties": {"Application_Type": "web"}}
    ...
    logger.warning(
        'Application Insights "%s" was created for this Function App. '
    ...
    update_app_settings(
        cmd,
        functionapp.resource_group,
        functionapp.name,
        ["APPINSIGHTS_INSTRUMENTATIONKEY={}".format(appinsights.instrumentation_key)],
    )


def _set_remote_or_local_git(
    cmd,
    webapp,
    resource_group_name,
    name,
...
def _validate_and_get_connection_string(cli_ctx, resource_group_name, storage_account):
    ...
    storage_properties = storage_client.storage_accounts.get_properties(sa_resource_group, storage_account)
    error_message = ""
    ...
    for e in ["blob", "queue", "table"]:
        if not getattr(endpoints, e, None):
            error_message = "Storage account '{}' has no '{}' endpoint. It must have table, queue, and blob endpoints all enabled".format(
                storage_account, e
            )  # pylint: disable=line-too-long
    ...
    connection_string = "DefaultEndpointsProtocol={};EndpointSuffix={};AccountName={};AccountKey={}".format(
        "https", endpoint_suffix, storage_account, keys[0]
...
def _check_zip_deployment_status(cmd, rg_name, name, deployment_status_url, authorization, timeout=None):
    ...
    while num_trials < total_trials:
        ...
        response = requests.get(
            deployment_status_url, headers=authorization, verify=not should_disable_connection_verify()
        )
        try:
        ...
        if res_dict.get("status", 0) == 3:
            ...
            raise CLIError(
                "Zip deployment failed. {}. Please run the command az webapp log deployment show "
    ...
    if res_dict.get("status", 0) != 4:
        _configure_default_logging(cmd, rg_name, name)
        raise CLIError(
            """Timeout reached by the command, however, the deployment operation
...
def add_hc(cmd, name, resource_group_name, namespace, hybrid_connection, slot=None):
    ...
    if not has_default_sender_key:
        ...
        hy_co_client.create_or_update_authorization_rule(
            hy_co_resource_group, namespace, hybrid_connection, "defaultSender", rights
        )
    ...
    hc = HybridConnection(
        service_bus_namespace=id_parameters[8],
    ...
    if slot is None:
        return_hc = web_client.web_apps.create_or_update_hybrid_connection(
            resource_group_name, name, namespace, hybrid_connection, hc
        )
    else:
        return_hc = web_client.web_apps.create_or_update_hybrid_connection_slot(
            resource_group_name, name, namespace, hybrid_connection, slot, hc
        )
...
def set_hc_key(cmd, plan, resource_group_name, namespace, hybrid_connection, key_type):
    ...
    asp_hy_co = web_client.app_service_plans.get_hybrid_connection(
        resource_group_name, plan, namespace, hybrid_connection
    )
    ...
    if not has_default_sender_key:
        rights = [AccessRights.send]
        hy_co_client.create_or_update_authorization_rule(
            relay_resource_group, namespace, hybrid_connection, "defaultSender", rights
        )
    ...
    apps = web_client.app_service_plans.list_web_apps_by_hybrid_connection(
        resource_group_name, plan, namespace, hybrid_connection
    )
    ...
    for x in apps:
        ...
        hc = HybridConnection(
            ...
            relay_name=hybrid_connection,
            relay_arm_uri=arm_uri,
            hostname=hostname,
            port=port,
            send_key_name="defaultSender",
            send_key_value=key,
        )
        web_client.web_apps.update_hybrid_connection(app_resource_group, app_name, namespace, hybrid_connection, hc)

    return web_client.app_service_plans.list_web_apps_by_hybrid_connection(
        resource_group_name, plan, namespace, hybrid_connection
    )
...
def remove_hc(cmd, resource_group_name, name, namespace, hybrid_connection, slot=None):
    ...
    if slot is None:
        ...
        return_hc = client.web_apps.delete_hybrid_connection_slot(
            resource_group_name, name, namespace, hybrid_connection, slot
        )
    return return_hc
...
def add_functionapp_vnet_integration(
    cmd, name, resource_group_name, vnet, subnet, slot=None, skip_delegation_check=False
):
    return _add_vnet_integration(cmd, name, resource_group_name, vnet, subnet, slot, skip_delegation_check)
...
def _add_vnet_integration(cmd, name, resource_group_name, vnet, subnet, slot=None, skip_delegation_check=False):
    ...
    if skip_delegation_check:
        logger.warning(
            "Skipping delegation check. Ensure that subnet is delegated to Microsoft.Web/serverFarms."
    ...
    _generic_site_operation(
        cmd.cli_ctx, resource_group_name, name, "begin_create_or_update", slot, client=client, extra_parameter=app
    )
...
def _vnet_delegation_check(cmd, subnet_subscription_id, vnet_resource_group, vnet_name, subnet_name):
    ...
    if get_subscription_id(cmd.cli_ctx).lower() != subnet_subscription_id.lower():
        logger.warning(
            "Cannot validate subnet in other subscription for delegation to Microsoft.Web/serverFarms."
            ' Missing delegation can cause "Bad Request" error.'
        )
        logger.warning(
            "To manually add a delegation, use the command: az network vnet subnet update "
            "--resource-group %s "
            "--name %s "
            "--vnet-name %s "
        ...
        if not delegated:
            ...
            vnet_client.subnets.begin_create_or_update(
                vnet_resource_group, vnet_name, subnet_name, subnet_parameters=subnetObj
            )
...
def _validate_subnet(cli_ctx, subnet, vnet, resource_group_name):
    ...
    if len(vnets) > 1:
        logger.warning(
            "Multiple virtual networks of name %s were found. Using virtual network with resource ID: %s. "
...
def webapp_up(
    ...
    plan=None,
    location=None,
    sku=None,  # pylint: disable=too-many-statements,too-many-branches
    os_type=None,
    runtime=None,
    dryrun=False,
    logs=False,
    ...
    if runtime:
        ...
        if not match:
            raise ValidationError(
                "{0} runtime '{1}' is not supported. Please check supported runtimes with: "
    ...
    runtime_version = (
        "{}|{}".format(language, version_used_create) if version_used_create != "-" else version_used_create
    )
    site_config = None
    ...
    if not _create_new_app:  # App exists, or App name unavailable
        ...
        if app_details is None:
            raise CLIError(
                "Unable to retrieve details of the existing app '{}'. Please check that the app "
        ...
        if resource_group_name is not None and (resource_group_name.lower() != current_rg.lower()):
            raise CLIError(
                "The webapp '{}' exists in ResourceGroup '{}' and does not "
                "match the value entered '{}'. Please re-run command with the "
                "correct parameters.".format(name, current_rg, resource_group_name)
            )
        ...
        if plan is not None and current_plan.lower() != plan.lower():
            raise CLIError(
                "The plan name entered '{}' does not match the plan name that the webapp is hosted in '{}'."
                "Please check if you have configured defaults for plan name and re-run command.".format(
                    plan, current_plan
                )
            )
        ...
        if current_os.lower() != os_name.lower():
            raise CLIError(
                "The webapp '{}' is a {} app. The code detected at '{}' will default to "
                "'{}'. Please create a new app "
                "to continue this operation. For more information on default behaviors, "
                "see https://docs.microsoft.com/cli/azure/webapp?view=azure-cli-latest#az_webapp_up.".format(
                    name, current_os, src_dir, os_name
                )
            )
    ...
    try:
        create_app_service_plan(
            ...
            per_site_scaling=False,
            sku=sku,
            number_of_workers=1 if _is_linux else None,
            location=loc,
    ...
    if _create_new_app:
        ...
        create_webapp(
            cmd, rg_name, name, plan, runtime_version if not html else None, using_webapp_up=True, language=language
        )
        _configure_default_logging(cmd, rg_name, name)
        ...
        if os_name.lower() == "linux" and site_config.linux_fx_version != runtime_version:
            if match and site_config.linux_fx_version != match.configs["linux_fx_version"]:
                logger.warning(
                    "Updating runtime version from %s to %s",
                    site_config.linux_fx_version,
                    match.configs["linux_fx_version"],
                )
...
def _set_webapp_up_default_args(cmd, rg_name, sku, plan, loc, name):
    with ConfiguredDefaultSetter(cmd.cli_ctx.config, True):
        logger.warning(
            "Setting 'az webapp up' default arguments for current directory. "
...
def _ping_scm_site(cmd, resource_group, name, instance=None):
    ...
    requests.get(
        scm_url + "/api/settings", headers=authorization, verify=not should_disable_connection_verify(), cookies=cookies
    )
...
def create_tunnel_and_session(cmd, resource_group_name, name, port=None, slot=None, timeout=None, instance=None):
    ...
    s = threading.Thread(
        target=_start_ssh_session, args=("localhost", tunnel_server.get_port(), ssh_user_name, ssh_user_password)
    )
    s.daemon = True
    s.start()
...
def _get_basic_headers(params):
    ...
    user_name, password = _get_site_credential(
        params.cmd.cli_ctx, params.resource_group_name, params.webapp_name, params.slot
    )
...
def _get_onedeploy_request_body(params):
    ...
    if params.src_path:
        ...
        try:
            ...
            raise CLIError(
                "Either '{}' is not a valid local file path or you do not have permissions to access it".format(
                    params.src_path
                )
            ) from e
    elif params.src_url:
        logger.info("Deploying from URL: %s", params.src_url)
        body = json.dumps({"packageUri": params.src_url})
    else:
        raise CLIError("Unable to determine source location of the artifact being deployed")
...
def _update_artifact_type(params):
    ...
    logger.warning(
        "Deployment type: %s. To override deployment type, please specify the --type parameter. "
        "Possible values: war, jar, ear, zip, startup, script, static",
...
def _make_onedeploy_request(params):
    ...
    if response.status_code == 409:
        raise CLIError(
            "Another deployment is in progress. Please wait until that process is complete before "
            "starting a new deployment. You can track the ongoing deployment at {}".format(deployment_status_url)
        )

    # check if an error occured during deployment
    if response.status_code:
        raise CLIError(
            "An error occured during deployment. Status Code: {}, Details: {}".format(
...
def _wait_for_webapp(tunnel_server):
    ...
    while True:
        ...
        if tries == 60:
            raise CLIError(
                "SSH timeout, your app must be running before"
...
def ssh_webapp(
    cmd, resource_group_name, name, port=None, slot=None, timeout=None, instance=None
...
def _configure_default_logging(cmd, rg_name, name):
    ...
    return config_diagnostics(
        cmd, rg_name, name, application_logging=True, web_server_logging="filesystem", docker_container_logging="true"
    )
...
def _verify_hostname_binding(cmd, resource_group_name, name, hostname, slot=None):
    ...
    for hostname_binding in hostname_bindings:
        ...
        if binding_name.lower() == hostname and (
            hostname_binding.host_name_type == "Verified" or hostname_binding.host_name_type == "Managed"
        ):
...
def update_host_key(cmd, resource_group_name, name, key_type, key_name, key_value=None, slot=None):
    ...
    client = web_client_factory(cmd.cli_ctx)
    if slot:
        return client.web_apps.create_or_update_host_secret_slot(
            resource_group_name, name, key_type, key_name, slot, key=key_info
        )
    return client.web_apps.create_or_update_host_secret(resource_group_name, name, key_type, key_name, key=key_info)
...
def update_function_key(cmd, resource_group_name, name, function_name, key_name, key_value=None, slot=None):
    ...
    client = web_client_factory(cmd.cli_ctx)
    if slot:
        return client.web_apps.create_or_update_function_secret_slot(
            resource_group_name, name, function_name, key_name, slot, key_info
        )
    return client.web_apps.create_or_update_function_secret(
        resource_group_name, name, function_name, key_name, key_info
    )
...
def add_github_actions(
    ...
    runtime=None,
    token=None,
    slot=None,  # pylint: disable=too-many-statements,too-many-branches
    branch="master",
    ...
    if site_availability.name_available or (
        not site_availability.name_available and site_availability.reason == "Invalid"
    ):
        raise ResourceNotFoundError(
            "The Resource 'Microsoft.Web/sites/%s' under resource group '%s' " "was not found." % (name, resource_group)
        )
    app_details = get_app_details(cmd, name)
    if app_details is None:
        raise ResourceNotFoundError(
    ...
    if resource_group is not None and (resource_group.lower() != current_rg.lower()):
        raise ResourceNotFoundError(
            "The webapp %s exists in ResourceGroup %s and does not match the "
    ...
    try:
        ...
        raise CLIError(
            "Could not authenticate to the repository. Please create a Personal Access Token and use "
    ...
    if runtime and app_runtime_string:
        if app_runtime_string.lower() != runtime.lower():
            logger.warning(
                "The app runtime: {app_runtime_string} does not match the runtime specified: "
    ...
    logger.warning(
        ...
        slot if slot else "production",
    )
    completed_workflow_file = _fill_workflow_template(
        content=workflow_template.decoded_content.decode(),
        name=name,
        branch=branch,
        slot=slot,
    ...
    try:
        ...
        if force:
            ...
            github_repo.update_file(
                path=file_path,
                message="Update workflow using Azure CLI",
                content=completed_workflow_file,
                sha=existing_workflow_file.sha,
                branch=branch,
            )
        else:
        ...
        github_repo.create_file(
            path=file_path, message="Create workflow using Azure CLI", content=completed_workflow_file, branch=branch
        )

    # Add publish profile to GitHub
    logger.warning("Adding publish profile to GitHub")
    _add_publish_profile_to_github(
        cmd=cmd,
        resource_group=resource_group,
        name=name,
        repo=repo,
        token=token,
        github_actions_secret_name=publish_profile_name,
        slot=slot,
    )
...
def remove_github_actions(
    ...
    repo,
    token=None,
    slot=None,  # pylint: disable=too-many-statements
    branch="master",
    login_with_github=False,
):
    if not token and not login_with_github:
    ...
    if app_details is None:
        raise CLIError(
            "Unable to retrieve details of the existing app %s. "
            "Please check that the app is a part of the current subscription" % name
        )
    current_rg = app_details.resource_group
    if resource_group is not None and (resource_group.lower() != current_rg.lower()):
        raise CLIError(
            "The webapp %s exists in ResourceGroup %s and does not match "
    ...
    file_name = (
        "{}_{}({}).yml".format(branch.replace("/", "-"), name.lower(), slot)
        if slot
        else "{}_{}.yml".format(branch.replace("/", "-"), name.lower())
    )
    dir_path = "{}/{}".format(".github", "workflows")
    file_path = "/{}/{}".format(dir_path, file_name)
    ...
    try:
        ...
        github_repo.delete_file(
            path=file_path,
            message="Removing workflow file, disconnecting github actions",
            sha=existing_workflow_file.sha,
            branch=branch,
        )
    ...
    if existing_publish_profile_name:
        ...
        _remove_publish_profile_from_github(
            cmd=cmd,
            resource_group=resource_group,
            name=name,
            repo=repo,
            token=token,
            github_actions_secret_name=existing_publish_profile_name,
            slot=slot,
        )

    # Remove site source control properties
    delete_source_control(cmd=cmd, resource_group_name=resource_group, name=name, slot=slot)
...
def _update_site_source_control_properties_for_gh_action(
    cmd, resource_group, name, token, repo=None, branch="master", slot=None
):
    if repo:
    ...
    site_source_control = show_source_control(cmd=cmd, resource_group_name=resource_group, name=name, slot=slot)
...
def _add_publish_profile_to_github(cmd, resource_group, name, repo, token, github_actions_secret_name, slot=None):
    ...
    publish_profile_bytes = _generic_site_operation(
        cmd.cli_ctx, resource_group, name, "list_publishing_profile_xml_with_secrets", slot, {"format": "WebDeploy"}
    )
    ...
    payload = {"encrypted_value": encrypted_github_actions_secret, "key_id": public_key["key_id"]}
...
def _get_app_runtime_info_helper(cmd, app_runtime, app_runtime_version, is_linux):
    ...
    if gh_props:
        if gh_props.get("github_actions_version"):
            if is_linux:
                return {"display_name": app_runtime, "github_actions_version": gh_props["github_actions_version"]}
