...
from ._constants import (
    DEFAULT_LOGICAPP_FUNCTION_VERSION,
...
def create_logicapp(
    ...
    if consumption_plan_location:
        ...
        if location is None:
            raise InvalidArgumentValueError("Location is invalid. Use: az logicapp list-consumption-locations")
    ...
    site_config.app_settings.append(
        NameValuePair(name="AzureFunctionsJobHost__extensionBundle__version", value="[1.*, 2.0.0)")
    )
    ...
    if (
        consumption_plan_location is None
    ...
    poller = client.web_apps.begin_create_or_update(resource_group_name, name, logicapp_def)
    logicapp = LongRunningOperation(cmd.cli_ctx)(poller)

    if consumption_plan_location and is_linux:
        logger.warning(
            "Your Linux logic app '%s', that uses a consumption plan has been successfully "
    ...
    if create_app_insights:
        try:
            ...
            logger.warning(
                "Error while trying to create and configure an Application Insights for the Logic App. "
...
def list_logicapp(cmd, resource_group_name=None):
    return list(
        filter(
            lambda x: x.kind is not None and "workflow" in x.kind.lower(), _list_app(cmd.cli_ctx, resource_group_name)
        )
    )
